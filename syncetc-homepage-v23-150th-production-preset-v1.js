/* syncetc-homepage-v23-150th-production-preset-v1.js - BEGIN */
/*
  Hosted SyncEtc Webflow asset.
  Update 95 / JS v23: 150th Aero production preset v1 and page-build pivot.
  Upload this file as:
  syncetc-homepage-v23-150th-production-preset-v1.js
...
/* syncetc_update_95_150th_production_preset_v1 - BEGIN */
(function () {
  const ROOT_ID = "syncetc-generated-homepage-v2";
  const LAB_ID = "syncetc-production-preset-workbench-v23";
  const STYLE_ID = "syncetc-production-preset-v23-style";
  const VERSION_LABEL = "JS v23-150th-production-preset-v1";
  const FILE_NAME = "syncetc-homepage-v23-150th-production-preset-v1.js";
  const STORAGE_KEY = "syncetc_production_preset_workbench_v23";

  const PRESETS = {
    "150th-aero-v1": {
      label: "150th Aero Style v1",
      status: "Active demo preset",
      description: "Primary sales/demo style. Aviation blue, clean club cards, high contrast, and conservative operating-page layout.",
      primary: "#12365a",
      accent: "#2f80c4",
      secondary: "#0f4d72",
      ...
      hero: "Aviation Blue / Club Operations",
      available: true
    },
    "classic-club-placeholder": {
      label: "Classic Club",
      status: "Placeholder",
      description: "Future warmer club style with traditional layout, reserved for a later preset pass.",
      primary: "#4a2f1a",
      accent: "#b28a47",
      secondary: "#6f4a24",
      surface: "rgba(255,255,255,.94)",
      ...
      radius: "18px",
      buttonRadius: "14px",
      shadow: "0 16px 38px rgba(74,47,26,.14)",
      hero: "Classic Club / Warm Traditional",
      available: false
    },
    "modern-ops-placeholder": {
      label: "Modern Ops",
      status: "Placeholder",
      description: "Future operational dashboard style for FBOs, schools, and fleet businesses.",
      primary: "#18222f",
      accent: "#38bdf8",
      secondary: "#2563eb",
      surface: "rgba(255,255,255,.94)",
      ...
      radius: "16px",
      buttonRadius: "10px",
      shadow: "0 18px 44px rgba(24,34,47,.16)",
      hero: "Modern Operations / Portal",
      available: false
    },
    "clean-portal-placeholder": {
      label: "Clean Portal",
      status: "Placeholder",
      description: "Future minimal portal style for customers who only need member/admin tools, not a full public site.",
      primary: "#27323a",
      accent: "#64748b",
      secondary: "#475569",
      ...
      radius: "14px",
      buttonRadius: "999px",
      shadow: "0 14px 34px rgba(39,50,58,.12)",
      hero: "Clean Portal / Minimal",
      available: false
    }
  };

  const PAGE_MODELS = {
    home: { label: "Home", title: "150th Aero Flying Club", subtitle: "Club operations, documents, events, and member tools in one polished SyncEtc-powered site.", modules: ["Events", "Gallery", "Resources", "Member Portal"] },
    events: { label: "Events", title: "Events and RSVP", subtitle: "Club meetings, fly-outs, maintenance windows, RSVP workflow, and member-only event context.", modules: ["Calendar", "RSVP", "Member notices", "Admin event tools"] },
    documents: { label: "Documents", title: "Documents and Resources", subtitle: "Rules, minutes, aircraft documents, oil reports, and controlled admin resources.", modules: ["Minutes", "Rules", "Aircraft docs", "Admin archive"] },
    gallery: { label: "Gallery", title: "Gallery and Club Media", subtitle: "Public-facing photos plus member/admin review workflows behind the scenes.", modules: ["Public gallery", "Upload", "Review", "Featured image"] },
    people: { label: "Roster", title: "Roster and People", subtitle: "Member directory, board contacts, permissions, and scoped profile information.", modules: ["Roster", "Profiles", "Board", "Access roles"] }
  };
  ...
  function getDefaultState() {
    return {
      ...
      layoutSide: "controls-left"
    };
  }

  function readState() {
    let stored = null;
    try { stored = JSON.parse(window.localStorage.getItem(STORAGE_KEY) || "null"); } catch (error) {}
    return Object.assign(getDefaultState(), stored || {});
  }

  function saveState(state) {
    try { window.localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch (error) {}
    const r = getRoot();
    if (r) r.setAttribute("data-se-production-preset-state", JSON.stringify(state));
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .syncetc-homepage-v2 {
        --se-prod-primary:#12365a;
        --se-prod-accent:#2f80c4;
        --se-prod-secondary:#0f4d72;
        --se-prod-surface:rgba(255,255,255,.94);
        --se-prod-bg:#eef5fa;
        --se-prod-radius:22px;
        --se-prod-button-radius:999px;
        --se-prod-shadow:0 18px 48px rgba(18,54,90,.16);
      }
      .syncetc-homepage-v2 .se-section,
      .syncetc-homepage-v2 .se-topbar,
      .syncetc-homepage-v2 .se-status { background:rgba(255,255,255,.94) !important; backdrop-filter:blur(16px); }
      .se-v23-version-bar { position:sticky; top:0; z-index:999997; background:var(--se-prod-primary); color:#fff; border-bottom:3px solid var(--se-prod-accent); padding:10px 14px; text-align:center; font:900 13px/1.2 Arial,sans-serif; letter-spacing:.02em; }
      .se-v23-version-badge { position:fixed; right:12px; top:12px; z-index:999999; background:var(--se-prod-primary); color:#fff; border:1px solid rgba(255,255,255,.58); border-radius:999px; padding:8px 11px; box-shadow:0 8px 24px rgba(0,0,0,.25); font:900 12px/1 Arial,sans-serif; pointer-events:none; }
      .se-v23-hidden-info { display:none !important; }
      .syncetc-homepage-v2.se-v23-show-full .se-v23-hidden-info { display:block !important; }
      .syncetc-homepage-v2 .se-v23-workbench { max-width:1180px; margin:18px auto; background:rgba(255,255,255,.96); border:1px solid rgba(18,54,90,.16); border-radius:24px; box-shadow:var(--se-prod-shadow); overflow:hidden; }
      .se-v23-head { display:flex; justify-content:space-between; gap:16px; align-items:flex-start; padding:18px 20px; background:linear-gradient(135deg,var(--se-prod-primary),var(--se-prod-secondary)); color:#fff; }
      .se-v23-head h2 { margin:0 0 4px; color:#fff; font-size:23px; line-height:1.05; letter-spacing:-.02em; }
      .se-v23-head p { margin:0; color:rgba(255,255,255,.88); max-width:760px; line-height:1.35; }
      .se-v23-badges { display:flex; flex-wrap:wrap; gap:7px; justify-content:flex-end; min-width:210px; }
      .se-v23-badges span { display:inline-flex; padding:7px 9px; border-radius:999px; background:rgba(255,255,255,.92); color:var(--se-prod-primary); font-size:11px; font-weight:950; white-space:nowrap; }
      .se-v23-body { display:grid; grid-template-columns:320px minmax(0,1fr); gap:0; background:#fff; }
      .se-v23-controls { border-right:1px solid rgba(18,54,90,.13); background:#f8fafc; padding:16px; }
      .se-v23-controls h3 { margin:0 0 8px; color:#162f47; font-size:14px; }
      .se-v23-control { margin-bottom:12px; padding:10px; border:1px solid rgba(18,54,90,.12); border-radius:14px; background:#fff; }
      .se-v23-control label { display:block; margin-bottom:6px; color:#162f47; font-size:12px; font-weight:900; }
      .se-v23-control select,
      .se-v23-control input[type="text"] { width:100%; box-sizing:border-box; border:1px solid rgba(18,54,90,.18); border-radius:10px; padding:8px 9px; background:#fff; color:#162f47; font:inherit; }
      .se-v23-control small { display:block; margin-top:6px; color:#5d7285; font-size:11px; line-height:1.25; }
      .se-v23-check { display:flex; gap:8px; align-items:flex-start; font-size:12px; font-weight:800; color:#162f47; margin:8px 0; }
      .se-v23-preview-wrap { padding:18px; background:linear-gradient(180deg,var(--se-prod-bg),#ffffff); }
      .se-v23-preview-label { display:flex; justify-content:space-between; gap:10px; align-items:center; margin-bottom:10px; color:#162f47; font-size:12px; font-weight:950; text-transform:uppercase; letter-spacing:.05em; }
      .se-v23-final-preview { border:1px solid rgba(18,54,90,.15); border-radius:var(--se-prod-radius); overflow:hidden; background:var(--se-prod-surface); box-shadow:var(--se-prod-shadow); }
      .se-v23-final-top { padding:12px 16px; display:flex; align-items:center; justify-content:space-between; gap:12px; border-bottom:1px solid rgba(18,54,90,.10); background:rgba(255,255,255,.88); }
      .se-v23-brand { display:flex; align-items:center; gap:10px; min-width:0; }
      .se-v23-mark { width:38px; height:38px; border-radius:13px; background:linear-gradient(135deg,var(--se-prod-primary),var(--se-prod-accent)); box-shadow:inset 0 0 0 1px rgba(255,255,255,.24); }
      .se-v23-brand strong { display:block; color:var(--se-prod-primary); font-size:15px; line-height:1.1; }
      .se-v23-brand span { display:block; color:#63788b; font-size:11px; margin-top:2px; }
      .se-v23-nav { display:flex; gap:7px; flex-wrap:wrap; justify-content:flex-end; }
      .se-v23-nav span { display:inline-flex; padding:7px 9px; border-radius:999px; border:1px solid rgba(18,54,90,.12); background:#fff; color:var(--se-prod-primary); font-size:11px; font-weight:900; }
      .se-v23-hero { padding:28px; display:grid; grid-template-columns:minmax(0,1.18fr) minmax(280px,.82fr); gap:22px; align-items:center; }
      .se-v23-hero-copy .eyebrow { color:var(--se-prod-accent); font-size:11px; font-weight:950; letter-spacing:.10em; text-transform:uppercase; margin-bottom:10px; }
      .se-v23-hero h1 { margin:0; color:var(--se-prod-primary); font-size:clamp(30px,4vw,52px); line-height:1.02; letter-spacing:-.04em; }
      .se-v23-hero p { margin:12px 0 0; color:#4e6578; line-height:1.55; font-size:15px; }
      .se-v23-actions { display:flex; gap:9px; flex-wrap:wrap; margin-top:18px; }
      .se-v23-actions span { display:inline-flex; padding:10px 13px; border-radius:var(--se-prod-button-radius); font-weight:950; font-size:12px; border:1px solid rgba(18,54,90,.15); }
      .se-v23-actions .primary { background:var(--se-prod-primary); color:#fff; border-color:var(--se-prod-primary); }
      .se-v23-actions .secondary { background:#fff; color:var(--se-prod-primary); }
      .se-v23-panel { min-height:230px; border-radius:calc(var(--se-prod-radius) - 5px); background:linear-gradient(135deg,var(--se-prod-primary),var(--se-prod-accent)); color:#fff; padding:20px; display:flex; flex-direction:column; justify-content:space-between; box-shadow:inset 0 0 0 1px rgba(255,255,255,.16); }
      .se-v23-panel strong { display:block; font-size:13px; letter-spacing:.08em; text-transform:uppercase; opacity:.85; }
      .se-v23-panel .big { margin-top:8px; font-size:30px; line-height:1.05; font-weight:950; letter-spacing:-.03em; }
      .se-v23-module-grid { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:10px; padding:0 28px 28px; }
      .se-v23-module { border:1px solid rgba(18,54,90,.12); border-radius:16px; padding:13px; background:#fff; min-height:92px; }
      .se-v23-module strong { display:block; color:var(--se-prod-primary); font-size:13px; margin-bottom:5px; }
      .se-v23-module span { display:block; color:#63788b; font-size:12px; line-height:1.35; }
      .se-v23-footer-note { padding:10px 16px; border-top:1px solid rgba(18,54,90,.10); color:#63788b; font-size:11px; text-align:center; background:rgba(255,255,255,.70); }
      .se-v23-future { margin-top:12px; border:1px dashed rgba(18,54,90,.24); border-radius:14px; padding:10px; background:#fff; color:#4e6578; font-size:12px; line-height:1.35; }
      .se-v23-future strong { color:var(--se-prod-primary); }
      .se-v23-export { display:flex; flex-wrap:wrap; gap:8px; margin-top:12px; }
      .se-v23-export button { border:1px solid var(--se-prod-primary); border-radius:999px; background:var(--se-prod-primary); color:#fff; padding:8px 11px; font-weight:950; cursor:pointer; }
      .se-v23-export button.secondary { background:#fff; color:var(--se-prod-primary); }
      .se-v23-export-status { margin-top:8px; padding:8px 10px; border-radius:12px; background:#eaf6ff; color:#12365a; font-size:12px; font-weight:900; }
      @media (max-width:980px) { .se-v23-body { grid-template-columns:1fr; } .se-v23-controls { border-right:none; border-bottom:1px solid rgba(18,54,90,.13); } .se-v23-hero { grid-template-columns:1fr; } .se-v23-module-grid { grid-template-columns:repeat(2,minmax(0,1fr)); } }
      @media (max-width:620px) { .se-v23-module-grid { grid-template-columns:1fr; } .se-v23-final-top { align-items:flex-start; flex-direction:column; } }
    `;
    document.head.appendChild(style);
  }
  ...
  function applyPresetVars(preset) {
    const r = getRoot();
    if (!r || !preset) return;
    r.style.setProperty("--se-prod-primary", preset.primary);
    r.style.setProperty("--se-prod-accent", preset.accent);
    r.style.setProperty("--se-prod-secondary", preset.secondary);
    r.style.setProperty("--se-prod-surface", preset.surface);
    r.style.setProperty("--se-prod-bg", preset.bg);
    r.style.setProperty("--se-prod-radius", preset.radius);
    r.style.setProperty("--se-prod-button-radius", preset.buttonRadius);
    r.style.setProperty("--se-prod-shadow", preset.shadow);
    r.style.setProperty("--se-primary", preset.primary);
    r.style.setProperty("--se-accent", preset.accent);
    r.style.setProperty("--se-bg", preset.bg);
  }

  function updateVersionMarkers() {
    let bar = document.getElementById("syncetc-v23-version-bar");
    if (!bar) {
      bar = document.createElement("div");
      bar.id = "syncetc-v23-version-bar";
      document.body.insertBefore(bar, document.body.firstChild);
    }
    bar.className = "se-v23-version-bar";
    bar.textContent = "SyncEtc Hosted JS " + VERSION_LABEL + " loaded from " + FILE_NAME;
    bar.setAttribute("data-syncetc-version", VERSION_LABEL);

    let badge = document.getElementById("syncetc-v23-version-badge");
    if (!badge) {
      badge = document.createElement("div");
      badge.id = "syncetc-v23-version-badge";
      document.body.appendChild(badge);
    }
    badge.className = "se-v23-version-badge";
    badge.textContent = "JS v23 loaded";
  ...
  function hideTechnicalPanels(state) {
    const r = getRoot();
    if (!r) return;
    r.classList.toggle("se-v23-show-full", !!state.showFullPrototype);
    [
      "[data-se-admin-preview]",
      ...
      if (el) el.classList.add("se-v23-hidden-info");
    });
  }

  function presetOptions(state) {
    return Object.keys(PRESETS).map(function (key) {
      const p = PRESETS[key];
      const label = p.available ? p.label : p.label + " - future";
      return '<option value="' + escapeHtml(key) + '"' + (state.presetKey === key ? ' selected' : '') + '>' + escapeHtml(label) + '</option>';
    }).join("");
  }

  function pageOptions(state) {
    return Object.keys(PAGE_MODELS).map(function (key) {
      return '<option value="' + escapeHtml(key) + '"' + (state.pageKey === key ? ' selected' : '') + '>' + escapeHtml(PAGE_MODELS[key].label) + '</option>';
    }).join("");
  }

  function renderPreview(state) {
    const preset = PRESETS[state.presetKey] || PRESETS["150th-aero-v1"];
    const page = PAGE_MODELS[state.pageKey] || PAGE_MODELS.home;
    const navItems = state.audienceKey === "admin" ? ["Dashboard", "People", "Documents", "Events", "Settings"] : state.audienceKey === "member" ? ["Dashboard", "Roster", "Events", "Resources"] : ["Home", "Info", "Aircraft", "Calendar", "Gallery"];
    const moduleCards = page.modules.map(function (item) {
      return '<article class="se-v23-module"><strong>' + escapeHtml(item) + '</strong><span>Preset-aware module slot for the ' + escapeHtml(page.label) + ' page.</span></article>';
    }).join("");
    return '<div class="se-v23-final-preview">' +
      '<div class="se-v23-final-top"><div class="se-v23-brand"><div class="se-v23-mark"></div><div><strong>150th Aero Flying Club</strong><span>' + escapeHtml(preset.label) + ' • ' + escapeHtml(state.audienceKey) + ' view</span></div></div><nav class="se-v23-nav">' + navItems.map(function (n) { return '<span>' + escapeHtml(n) + '</span>'; }).join("") + '</nav></div>' +
      '<section class="se-v23-hero"><div class="se-v23-hero-copy"><div class="eyebrow">' + escapeHtml(preset.hero) + '</div><h1>' + escapeHtml(page.title) + '</h1><p>' + escapeHtml(page.subtitle) + '</p><div class="se-v23-actions"><span class="primary">View dashboard</span><span class="secondary">Contact us</span></div></div><aside class="se-v23-panel"><div><strong>Selected preset</strong><div class="big">' + escapeHtml(preset.label) + '</div></div><div>' + escapeHtml(preset.description) + '</div></aside></section>' +
      '<section class="se-v23-module-grid">' + moduleCards + '</section>' +
      '<div class="se-v23-footer-note">Powered by SyncEtc • 150th Aero Style v1 • production demo direction</div>' +
      '</div>';
  }
  ...
  function exportPayload(state) {
    const preset = PRESETS[state.presetKey] || PRESETS["150th-aero-v1"];
    return {
      export_type: "syncetc_production_preset_demo_state",
      version: VERSION_LABEL,
      file_name: FILE_NAME,
      exported_at: new Date().toISOString(),
      immediate_product_direction: "Build one polished 150th Aero demo site first; retain preset bones for later customer/style expansion.",
      active_preset: state.presetKey,
      active_page: state.pageKey,
      audience: state.audienceKey,
      preset_snapshot: preset,
      future_customer_facing_model: "Curated preset picker first; detailed builder stays internal/SyncEtc controlled until product is mature.",
      future_presets: Object.keys(PRESETS).filter(function (key) { return key !== "150th-aero-v1"; })
    };
  }
  ...
  function renderWorkbench() {
    ...
    wb.innerHTML = '<div class="se-v23-head"><div><h2>150th Aero Production Preset v1</h2><p>Build one polished customer site first. Preset bones remain in place so SyncEtc can add more vetted styles later without exposing breakable raw design controls to early customers.</p></div><div class="se-v23-badges"><span>JS v23</span><span>150th preset active</span><span>page-build pivot</span></div></div>' +
      '<div class="se-v23-body"><aside class="se-v23-controls"><h3>Plain controls</h3>' +
        '<div class="se-v23-control"><label>Customer style preset</label><select data-se-v23-field="presetKey">' + presetOptions(state) + '</select><small>Early customers choose from curated presets. Only 150th Aero v1 is production-ready right now.</small></div>' +
        '<div class="se-v23-control"><label>Preview page type</label><select data-se-v23-field="pageKey">' + pageOptions(state) + '</select><small>This lets us design actual pages next, not endless style knobs.</small></div>' +
        '<div class="se-v23-control"><label>Audience preview</label><select data-se-v23-field="audienceKey"><option value="public"' + (state.audienceKey === 'public' ? ' selected' : '') + '>Public</option><option value="member"' + (state.audienceKey === 'member' ? ' selected' : '') + '>Member</option><option value="admin"' + (state.audienceKey === 'admin' ? ' selected' : '') + '>Admin</option></select><small>Future menus and modules will be scoped by auth/RLS.</small></div>' +
        '<div class="se-v23-control"><label>Testing visibility</label><label class="se-v23-check"><input type="checkbox" data-se-v23-check="showFullPrototype"' + (state.showFullPrototype ? ' checked' : '') + '> Show lower technical prototype panels</label><label class="se-v23-check"><input type="checkbox" data-se-v23-check="showFuturePresets"' + (state.showFuturePresets ? ' checked' : '') + '> Show future preset placeholders</label><small>Default stays focused on selling/building one real site.</small></div>' +
        '<div class="se-v23-export"><button type="button" data-se-v23-action="export">Export preset/page JSON</button><button type="button" class="secondary" data-se-v23-action="reset">Reset</button></div><div data-se-v23-status class="se-v23-export-status" style="display:none"></div>' +
        (state.showFuturePresets ? '<div class="se-v23-future"><strong>Future preset slots:</strong> Classic Club, Modern Ops, Clean Portal. These are intentionally placeholders until the 150th demo site is complete.</div>' : '') +
      '</aside><main class="se-v23-preview-wrap"><div class="se-v23-preview-label"><span>Final-site style preview</span><span>' + escapeHtml(preset.status) + '</span></div>' + renderPreview(state) + '</main></div>';
  }

  function bind() {
    if (document.documentElement.getAttribute("data-se-v23-bound") === "true") return;
    document.documentElement.setAttribute("data-se-v23-bound", "true");
    document.addEventListener("change", function (event) {
      const field = event.target.closest("[data-se-v23-field]");
      const check = event.target.closest("[data-se-v23-check]");
      if (!field && !check) return;
      const state = readState();
      if (field) state[field.getAttribute("data-se-v23-field")] = field.value;
      if (check) state[check.getAttribute("data-se-v23-check")] = check.checked;
      saveState(state);
      renderWorkbench();
    });
    document.addEventListener("click", function (event) {
      const action = event.target.closest("[data-se-v23-action]");
      if (!action) return;
      const state = readState();
      const kind = action.getAttribute("data-se-v23-action");
      if (kind === "reset") {
      ...
      if (kind === "export") {
        ...
        downloadJson("syncetc-v23-150th-production-preset-demo.json", payload);
        const wb = document.getElementById(LAB_ID);
        const status = wb && wb.querySelector("[data-se-v23-status]");
        if (status) { status.style.display = "block"; status.textContent = "Exported local preset/page JSON. No database write occurred."; }
      }
    });
  }
  ...
  function boot() {
    ...
    console.log("SYNCETC UPDATE 95 PRODUCTION PRESET V1 LOADED", VERSION_LABEL, FILE_NAME);
  }

  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", boot);
  else boot();
})();
/* syncetc_update_95_150th_production_preset_v1 - END */

/* syncetc-homepage-v23-150th-production-preset-v1.js - END */
