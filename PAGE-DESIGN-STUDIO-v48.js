...
/* PAGE-DESIGN-STUDIO-v48.js | no fake data preview contract | Generated: 2026-06-01 */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v48";
...
var STYLE_ID="syncetc-design-studio-v48-style";
...
function activePageRegistry(){
  ...
  if(!reg)return null;
  if(ctx.enabled_pages&&ctx.enabled_pages.length&&ctx.enabled_pages.indexOf(page)<0)return null;
  return reg;
}

function registryPageOptions(){
  var ctx=activeCustomerContext();
  var enabled=ctx.enabled_pages&&ctx.enabled_pages.length?ctx.enabled_pages:Object.keys(SYNCETC_PAGE_REGISTRY);
  return enabled.map(function(key){return SYNCETC_PAGE_REGISTRY[key];}).filter(Boolean);
}
...
function ensureRegistrySelection(){
  ...
  if(!state.sidePageKey || !SYNCETC_PAGE_REGISTRY[state.sidePageKey] || (ctx.enabled_pages||[]).indexOf(state.sidePageKey)<0){
...
function renderPageOptions(){
  ...
  return options.map(function(p){
    return '<option value="'+esc(p.page_key)+'" '+(state.sidePageKey===p.page_key?'selected':'')+'>'+esc(p.label)+'</option>';
  }).join("");
...
function realPageValuesFor(prefix){
  ...
  Object.keys(vals||{}).forEach(function(k){
    if(k.indexOf(prefix+".")===0 && clean(vals[k]))out[k]=vals[k];
  });
  return out;
}
