/* PAGE-EVENT-RSVP-v4.js - BEGIN */
(function(){
"use strict";

var VERSION="PAGE-EVENT-RSVP-v4";
...
function scriptBase(){
  ...
  for(var i=scripts.length-1;i>=0;i--){
    ...
    if(src.indexOf("PAGE-EVENT-RSVP-v4.js")>-1){
...
function loadComponents(){
  ...
  COMPONENT_FILES.forEach(function(file){
    p=p.then(function(){return loadScript(base+file+"?v=event-rsvp-v4");});
  });
  return p;
}
...
function getAuthToken(){
  ...
  /* Prefer actual Supabase auth session if a client is present. */
  try{
    if(window.supabase&&window.supabase.auth&&window.supabase.auth.getSession){
      return window.supabase.auth.getSession().then(function(res){
        ...
        if(!t)return waitForToken(2500);
        return t;
      }).then(function(t){
        if(t)setUserFromToken(t);
        return t;
      }).catch(function(){
        return waitForToken(2500).then(function(t){if(t)setUserFromToken(t);return t;});
      });
    }
  }catch(e){}

  /* Then look for Supabase session in storage. */
  ...
  /* Ask SyncEtc auth layer to initialize, then re-check storage. */
  try{
    if(C().AuthContext&&C().AuthContext.init){
      return Promise.resolve(C().AuthContext.init({activeCustomerKey:state.customerKey}))
        .then(function(){return waitForToken(3000);})
        .then(function(t){if(t)setUserFromToken(t);return t;})
        .catch(function(){return waitForToken(1500).then(function(t){if(t)setUserFromToken(t);return t;});});
    }
  }catch(e){}

  return waitForToken(2500).then(function(t){
    if(t)setUserFromToken(t);
    return t;
  });
}
...
function openLoginModalForRsvp(){
  ...
  setMessage("RSVP needs a fresh login. Use the SyncEtc sign-in control, then click Refresh.", "error");
}
...
function callRsvpFunction(payload){
  return getAuthToken().then(function(token){
    ...
    if(!state.authToken||!isLikelySupabaseJwt(state.authToken)){
      ...
      throw new Error("RSVP needs a fresh SyncEtc login. Sign in in the popup, then the page will retry automatically.");
    }
    ...
    return resp.text().then(function(txt){
      ...
      if(!resp.ok||data.ok===false){
        if(resp.status===401){
          openLoginModalForRsvp();
          throw new Error("RSVP request failed: 401. Supabase did not accept the current login token. Sign in again in the popup, then retry RSVP.");
        }
...
function styleHtml(){
  return '<style id="PAGE-EVENT-RSVP-v4-style">'+
