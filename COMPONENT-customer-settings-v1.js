...
function installStyles(){
  U().installStyle("COMPONENT-customer-settings-v1-style",`
    ...
    .se-customer-settings-note{margin:0 0 12px;color:#657789;font-size:12px;line-height:1.4;font-weight:700}
    ...
    @media(max-width:900px){.se-customer-settings-row{grid-template-columns:1fr}}
  `);
}
...
function render(ctx){
  ...
  return `<section class="se-customer-settings"><div class="se-customer-settings-shell ${open?"is-open":""}" data-se-customer-settings>
    <div class="se-customer-settings-top">
      <div class="se-customer-settings-title"><h2>Customer Settings</h2><span class="se-customer-settings-pill">customer admin layer</span><span class="se-customer-settings-pill">${U().esc(c.shortName||c.customerName||"Customer")}</span></div>
...
function save(api,root){
  ...
  fetch("https://ocdaohkiwonjmirqkjww.supabase.co/functions/v1/syncetc-site-settings-action",{
    ...
    body:JSON.stringify({action:"save_customer_site_settings",payload:buildPayload(api)})
  ...
  .then(function(){setStatus(root,"Saved customer settings.","good");})
...
function bind(api,root){
  ...
  box.addEventListener("click",function(e){
    ...
    if(e.target.closest("[data-cs-save]")){save(api,box);return;}
...
window.SyncEtc.Components.CustomerSettings={version:VERSION,render:render,bind:bind,installStyles:installStyles};
})();
