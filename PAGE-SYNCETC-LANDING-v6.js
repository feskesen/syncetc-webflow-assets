/* PAGE-SYNCETC-LANDING-v6.js | Uses true shared SyncEtc header component | Generated: 2026-06-03 */
(function(){
  "use strict";

  var VERSION="PAGE-SYNCETC-LANDING-v6";
  var ROOT_ID="syncetc-webflow-mount";
  var STYLE_ID="syncetc-landing-v6-style";
  ...
  function render(){requireToolHeader();installStyle();var root=document.getElementById(ROOT_ID);if(!root)return;var logged=isAuthed();root.innerHTML='<main class="se-hub">'+window.SyncEtcToolHeader.render({active:"hub"})+'<section class="se-panel"><div class="se-panel-inner">'+(logged?'<div class="se-signout-row"><button class="se-pill" type="button" data-se-action="signout">Sign out</button></div>':'')+(state.notice?'<div class="se-msg ok">'+esc(state.notice)+'</div>':'')+(state.error?'<div class="se-msg err">'+esc(state.error)+'</div>':'')+(logged?renderTools():renderLoggedOut())+'<div class="se-foot">'+esc(VERSION)+'</div></div></section></main>';bind();}
  ...
  window.SYNCETC_LANDING_V6_STATE=function(){var out={version:VERSION,authed:isAuthed(),authEmail:state.authEmail,usesComponent:!!window.SyncEtcToolHeader};console.log("SYNCETC_LANDING_V6_STATE",out);return out;};
  function start(){loadAuthSession();render();}
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",start);else start();
})();

/* PAGE-SYNCETC-LANDING-v6.js - END */
