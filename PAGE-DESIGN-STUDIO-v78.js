...
/* PAGE-DESIGN-STUDIO-v78.js | customer page scoping and safe empty state | Generated: 2026-06-02 */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v78";
...
var STYLE_ID="syncetc-design-studio-v78-style";
...
/* v78 customer/page scoping helpers - BEGIN */
function v78CanonicalCustomerKey(value){
  return normalizeCustomerKey(value||"");
}

function v78ActiveCustomerKey(){
  var ctx=activeCustomerContext&&activeCustomerContext();
  return v78CanonicalCustomerKey((ctx&&ctx.customer_key)||state.customer_key||"");
}

function v78RowBelongsToActiveCustomer(row){
  var rowKey=clean(row&&row.customer_key);
  if(!rowKey)return false;
  return v78CanonicalCustomerKey(rowKey)===v78ActiveCustomerKey();
}

function v78ClearScopedPreviewData(reason){
  state.pageBridge=null;
  state.pageBridgeLoading=false;
  state.pageBridgeError="";
  state.pageBridgeRequestedFor="";
  state.assetRecords=[];
  state.aircraftRecords=[];
  state.infoRecords=null;
  state.faqs=[];
  state.officers=[];
  ...
  if(state.data){
    state.data.page_bridge=null;
    state.data.page_settings=null;
    state.data.records={};
    state.data.assets=[];
    state.data.aircraft=[];
    state.data.info_records=null;
  }
  state.lastScopedResetReason=reason||"";
}

function v78DefaultPageForActiveCustomer(){
  var ctx=activeCustomerContext&&activeCustomerContext();
  var preferred=normalizePageKey((ctx&&ctx.default_page)||"assets");
  if(preferred&&isPageUnlockedForCustomer(preferred))return preferred;
  var pages=(ctx&&ctx.enabled_pages)||[];
  for(var i=0;i<pages.length;i++){
    var key=normalizePageKey(pages[i]);
    if(key&&SYNCETC_PAGE_REGISTRY[key])return key;
  }
  return "assets";
}
/* v78 customer/page scoping helpers - END */
...
function defaultCustomerName(customerKey){
  ...
  if(customerKey==="demo_flying_club"||customerKey==="demo-flying-club")return "Demo Flying Club";
...
function pageRows(){
  return arr(state.data&&state.data.customer_page_settings).filter(v78RowBelongsToActiveCustomer);
}
...
function applyPageReadBridgePayload(payload){
  var ctx=activeCustomerContext&&activeCustomerContext();
  var reg=activePageRegistry&&activePageRegistry();
  var payloadCustomer=v78CanonicalCustomerKey(payload&&payload.customer_key);
  var activeCustomer=v78CanonicalCustomerKey(ctx&&ctx.customer_key);
  var payloadPage=normalizePageKey(payload&&payload.page_key);
  var activePage=normalizePageKey(reg&&reg.page_key);

  if(payload&&payload.ok!==false&&(payloadCustomer!==activeCustomer||payloadPage!==activePage)){
    state.pageBridge=null;
    state.pageBridgeError="Ignored stale page-read payload for "+payloadCustomer+"/"+payloadPage+" while active preview is "+activeCustomer+"/"+activePage+".";
    state.pageBridgeLoading=false;
    return;
  }
...
function connectedAssetRecords(){
  ...
  found=arr(found).filter(function(row){
    var key=clean((row&&row.customer_key)||(row&&row.raw&&row.raw.customer_key));
    if(!key)return false;
    return v78CanonicalCustomerKey(key)===v78ActiveCustomerKey();
  });
  return normalizeAssetRecords ? normalizeAssetRecords(found) : found;
}
...
function ensureBridgeForCurrentPage(){
  ...
  v78ClearScopedPreviewData("bridge-load:"+key);
...
function bindFrame(){
  ...
  if(!window.__SYNCETC_DS_ESC_CLOSE_V78){
    window.__SYNCETC_DS_ESC_CLOSE_V78=true;
  ...
  if(pageSelect){
    pageSelect.addEventListener('change',function(){
      ...
      v78ClearScopedPreviewData("page-switch");
  ...
  r.querySelectorAll('[data-ds-select]').forEach(function(sel){
    sel.addEventListener('change',function(){
      ...
      if(field==='customer'){
        ...
        state.customer_key=normalizeCustomerKey(sel.value);
        state.sidePageKey=v78DefaultPageForActiveCustomer();
        v78ClearScopedPreviewData("customer-switch");
        loadCustomer();
        return;
      }
...
window.SYNCETC_V78_CLOSE_WIRING_STATE=function(){
  var out={
    version:VERSION,
    sideOpen:state.sideOpen,
    sideDirty:state.sideDirty,
    sideAppliedDirty:state.sideAppliedDirty,
    escCloseInstalled:!!window.__SYNCETC_DS_ESC_CLOSE_V78,
  ...
  console.log('SYNCETC_V78_CLOSE_WIRING_STATE',out);
  return out;
};
...
window.SYNCETC_V78_SAVE_STATE=function(){
  var values=v77SaveValuesFromAppliedState();
  var target=v77SaveTargetState(values);
  var out={
    version:VERSION,
  ...
  console.log("SYNCETC_V78_SAVE_STATE",out);
  return out;
};
/* v77 save diagnostic - END */


/* v78 customer/page scoping diagnostic - BEGIN */
window.SYNCETC_V78_SCOPE_STATE=function(){
  var ctx=activeCustomerContext&&activeCustomerContext();
  var reg=activePageRegistry&&activePageRegistry();
  var out={
    version:VERSION,
    ...
    filteredPageRows:pageRows().map(function(r){return {customer_key:r.customer_key,page_key:r.page_key,page_label:r.page_label};}),
    pageBridgeCustomer:state.pageBridge&&state.pageBridge.customer_key,
    pageBridgePage:state.pageBridge&&state.pageBridge.page_key,
    assetRecordCount:connectedAssetRecords().length,
    lastScopedResetReason:state.lastScopedResetReason||""
  };
  console.log("SYNCETC_V78_SCOPE_STATE",out);
  return out;
};
/* v78 customer/page scoping diagnostic - END */
