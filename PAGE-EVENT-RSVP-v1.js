/* PAGE-EVENT-RSVP-v1.js - BEGIN */
(function(){
"use strict";

var VERSION="PAGE-EVENT-RSVP-v1";
...
function scriptBase(){
  ...
  for(var i=scripts.length-1;i>=0;i--){
    ...
    if(src.indexOf("PAGE-EVENT-RSVP-v1.js")>-1){
...
function loadComponents(){
  ...
  COMPONENT_FILES.forEach(function(file){
    p=p.then(function(){return loadScript(base+file+"?v=event-rsvp-v1");});
  });
  return p;
}
...
function getAuthToken(){
  try{
    if(C().AuthContext&&C().AuthContext.getSession){
      var s=C().AuthContext.getSession();
      if(s&&s.access_token)return Promise.resolve(s.access_token);
      if(s&&s.session&&s.session.access_token)return Promise.resolve(s.session.access_token);
    }
  }catch(e){}
  ...
  try{
    if(C().AuthContext&&C().AuthContext.currentUser){
      var u=C().AuthContext.currentUser();
      state.user=u||null;
    }
  }catch(e){}
...
function userProfileForPayload(){
  ...
  var email=clean(user.email||user.auth_email||user.member_email);
  var meta=user.user_metadata||user.metadata||{};
  return {
    member_name:clean(meta.full_name||meta.name||user.name||email||"Member"),
    member_email:email,
    member_type:clean(meta.member_type||meta.memberType||""),
    memberstack_id:clean(meta.memberstack_id||meta.memberstackId||"")
  };
}
...
function callRsvpFunction(payload){
  return getAuthToken().then(function(token){
    ...
    if(!state.authToken){
      throw new Error("You must be logged in before using RSVP.");
    }
...
function styleHtml(){
  return '<style id="PAGE-EVENT-RSVP-v1-style">'+
      ...
      '<div class="syncetc-rsvp-shell">'+
        '<section class="syncetc-rsvp-card">'+
          ...
          '<div class="syncetc-rsvp-body">'+
            ...
            '<div class="syncetc-rsvp-version">'+esc(VERSION)+'</div>'+
          '</div>'+
        '</section>'+
      '</div>'+
    '</main>';
