...
(function () {
  ...
  function installDrawerStyles(){
    window.SyncEtc.Components.Utils.installStyle("COMPONENT-site-shell-v14-drawer-style",`
  ...
  function create(mountId,options){
    var state=Object.assign({customerKey:"demo_flying_club",customerName:"Demo Flying Club",pageKey:"events",audience:"public",viewAs:"public",showControls:true,showBanner:false,adminReady:false,siteEditorOpen:false,customerSettingsOpen:false,drawerOpen:false,drawerTab:"site",local:{},version:VERSION,lastPageHtml:""},options||{});
    ...
    function renderAdminLayerOnly(){
      var shell=document.querySelector("#"+CSS.escape(mountId)+" .syncetc-shell");
      if(!shell)return;
      var existing=shell.querySelector(".syncetc-admin-layer-root");
      if(existing)existing.remove();
      var admin=document.createElement("div");
      admin.className="syncetc-admin-layer-root";
      admin.innerHTML=drawerHtml(customer(),state.audience||"public");
    ...
    if(!authListenerInstalled){
      ...
      function activateAdminLayer(){
        if(state.adminReady)return;
        state.adminReady=true;
        try{
          if(window.SyncEtc.SecurityContext&&window.SyncEtc.SecurityContext.refresh){
            window.SyncEtc.SecurityContext.refresh(state.customerKey||"demo_flying_club").then(function(){renderAdminLayerOnly();}).catch(function(){renderAdminLayerOnly();});
          }else{
            renderAdminLayerOnly();
          }
        }catch(e){renderAdminLayerOnly();}
      }
      try{document.addEventListener("syncetc:auth-soft-change",function(){if(state.adminReady)renderAdminLayerOnly();});}catch(e){}
      setTimeout(activateAdminLayer,5000);
    }
    var api={render:render,customer:customer,loadCustomer:loadCustomer,getState:getState,updateState:updateState,setLocal:setLocal,setViewAs:setViewAs,setCustomerKey:setCustomerKey,setEditorOpen:setEditorOpen,setCustomerSettingsOpen:setCustomerSettingsOpen,setDrawerOpen:setDrawerOpen,setDrawerTab:setDrawerTab,getSavePayload:getSavePayload,refreshSecurityAndRender:refreshSecurityAndRender,renderAdminLayerOnly:renderAdminLayerOnly,setAdminReady:function(value){state.adminReady=!!value;renderAdminLayerOnly();}};
    return api;
  }
  window.SyncEtc.Components.SiteShell={create:create,version:VERSION};
})();
/* COMPONENT-site-shell-v14.js - END */
