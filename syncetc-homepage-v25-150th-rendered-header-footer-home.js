/* syncetc-homepage-v25-150th-rendered-header-footer-home.js - BEGIN */
(function () {
  "use strict";

  const VERSION = "JS v25-150th-rendered-header-footer-home";
  const FILE_NAME = "syncetc-homepage-v25-150th-rendered-header-footer-home.js";
  const MOUNT_ID = "syncetc-webflow-mount";
  const ROOT_ID = "syncetc-v25-root";
  const STYLE_ID = "syncetc-v25-style";
  ...
  const CUSTOMER_DEFAULTS = {
    "150th Aero Flying Club": {
      ...
      featuredImageUrl: "https://ocdaohkiwonjmirqkjww.supabase.co/storage/v1/object/public/public-site-assets/150thAero/HOME-p-130x130q80.png",
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .syncetc-v25 { min-height:100vh; font-family:Arial, Helvetica, sans-serif; color:var(--aero-text); background:linear-gradient(rgba(240,247,252,.78),rgba(240,247,252,.80)), var(--se-customer-bg); background-position:center; background-size:cover; background-attachment:fixed; padding:10px 0 48px; box-sizing:border-box; }
      .syncetc-v25 * { box-sizing:border-box; }
      .se-v25-version { position:fixed; right:12px; top:12px; z-index:999999; padding:8px 11px; border-radius:999px; background:#12365a; color:#fff; font-size:12px; font-weight:900; box-shadow:0 8px 24px rgba(0,0,0,.25); pointer-events:none; }
      .se-v25-topline { max-width:1180px; margin:0 auto 8px; padding:0 18px; display:flex; justify-content:space-between; align-items:center; gap:10px; color:#12365a; font-size:11px; font-weight:900; }
      .se-v25-topline span { background:rgba(255,255,255,.72); border:1px solid var(--aero-border); border-radius:999px; padding:6px 9px; }
      ...
      .aero-marquee { max-width:1180px; margin:-4px auto 16px; padding:0 18px; overflow:hidden; }
      .aero-marquee-view { overflow:hidden; position:relative; min-height:38px; }
      ...
      .aero-marquee-plane { height:34px; width:auto; margin-right:12px; flex:0 0 auto; filter:drop-shadow(0 4px 6px rgba(12,38,64,.18)); }
      .aero-marquee-text { display:inline-flex; align-items:center; padding:5px 14px; border-radius:2px; background:rgba(207,220,228,.82); border:1px solid rgba(18,54,90,.30); color:#31465a; font-size:14px; font-weight:800; box-shadow:0 3px 8px rgba(12,38,64,.10); }
      @keyframes aeroMarqueeTrackMovePause { 0%{transform:translateX(100vw);} 18%{transform:translateX(calc(50vw - 50%));} 77%{transform:translateX(calc(50vw - 50%));} 100%{transform:translateX(-120%);} }
      ...
      .homepage-featured-slot { width:100%; min-height:320px; max-height:54vh; display:flex; align-items:center; justify-content:center; padding:14px; background:linear-gradient(180deg,rgba(234,245,255,.92),rgba(255,255,255,.86)); border:1px solid rgba(18,54,90,.13); border-radius:18px; overflow:hidden; }
      .homepage-featured-image { display:block; max-width:100%; max-height:50vh; width:auto; height:auto; object-fit:contain; border-radius:16px; box-shadow:0 12px 30px rgba(12,38,64,.22); background:#fff; }
  ...
  function renderMarquee(base) {
    ...
    const img = base.bannerTowUrl ? '<img class="aero-marquee-plane" src="' + esc(base.bannerTowUrl) + '" alt="">' : '';
    return `<div class="aero-marquee"><div class="aero-marquee-view"><div class="aero-marquee-track">${img}<div class="aero-marquee-text">${esc(base.announcement)}</div></div></div></div>`;
  }
  ...
  function renderHome(base) {
    return `<div class="site-page-wrap"><section class="aero-home-shell">
      ...
      <main class="aero-home-main">
        <section class="aero-featured-card"><div class="aero-section-label">From Our Gallery</div><div class="aero-featured-header"><div><h2>Featured Photo</h2><p>A rotating look at Club aircraft, members, destinations, and flying activity.</p></div><a href="#" class="aero-button aero-button-secondary">View Full Gallery</a></div><div class="homepage-featured-slot"><img class="homepage-featured-image" src="${esc(base.featuredImageUrl)}" alt="Featured photo"></div></section>
  ...
  function render() {
    ...
    mount.innerHTML = `<div id="${ROOT_ID}" class="syncetc-v25" style="--se-customer-bg:url('${cssUrl(base.backgroundUrl)}')"><div class="se-v25-version">JS v25 loaded</div><div class="se-v25-topline"><span>SyncEtc versioned renderer · ${esc(FILE_NAME)}</span><span>${esc(state.customer)} · ${esc(currentPageLabel())}</span></div>${renderWorkbench(base)}${renderHeader(base)}${renderMarquee(base)}${state.page === 'home' ? renderHome(base) : renderPageModules(base)}${renderFooter(base)}</div>`;
  }
  ...
  function exportJson() {
    const base = currentBase();
    const data = { export_type:"syncetc_customer_display_settings_prototype", version:VERSION, file_name:FILE_NAME, exported_at:new Date().toISOString(), customer:state.customer, page:state.page, audience:state.audience, controls_side:state.controlsSide, show_banner:state.showBanner, current_customer_assets:{ header_logo_url:base.headerLogoUrl, footer_logo_url:base.footerLogoUrl, homepage_background_url:base.backgroundUrl, homepage_fallback_image_url:base.featuredImageUrl, banner_tow_image_url:base.bannerTowUrl }, local_overrides:state.local };
    const blob = new Blob([JSON.stringify(data, null, 2)], {type:"application/json;charset=utf-8"});
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "syncetc-v25-display-settings-" + Date.now() + ".json";
    document.body.appendChild(a); a.click();
    setTimeout(function(){ URL.revokeObjectURL(url); a.remove(); }, 500);
  }

  function boot() { render(); bind(); console.log("SYNCETC UPDATE 97 LOADED", VERSION, FILE_NAME); }
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", boot); else boot();
})();
/* syncetc-homepage-v25-150th-rendered-header-footer-home.js - END */
