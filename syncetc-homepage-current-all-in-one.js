...
/*
  ...
  Upload this single file to:
  assets/syncetc-homepage-current-all-in-one.js

  Webflow should only need the small loader embed:
  <div id="syncetc-webflow-mount"></div>
  <script src="https://feskesen.github.io/syncetc-webflow-assets/assets/syncetc-homepage-current-all-in-one.js"></script>
*/

(function syncetcInjectShell() {
  const STYLE_ID = "syncetc-homepage-v12-style";
...
/* syncetc_current_public_preview_mode_toggle_v13 - BEGIN */
...
.syncetc-homepage-v2.se-public-preview-mode [data-se-health-strip],
.syncetc-homepage-v2.se-public-preview-mode [data-se-status],
.syncetc-homepage-v2.se-public-preview-mode [data-se-admin-preview],
.syncetc-homepage-v2.se-public-preview-mode [data-se-navigation-preview],
.syncetc-homepage-v2.se-public-preview-mode [data-se-backbone-preview],
.syncetc-homepage-v2.se-public-preview-mode [data-se-deployment-access-preview],
.syncetc-homepage-v2.se-public-preview-mode [data-se-boundary-preview] {
  display: none !important;
}

.syncetc-homepage-v2.se-public-preview-mode {
  padding-top: 22px;
}

.syncetc-homepage-v2.se-public-preview-mode .se-topbar {
  margin-bottom: 16px;
}

.syncetc-homepage-v2.se-public-preview-mode .se-customer-subtitle::after {
  content: " • public preview mode";
}
/* syncetc_current_public_preview_mode_toggle_v13 - END */
";
  ...
  if (!document.getElementById(ROOT_ID)) {
    ...
    const mount = document.getElementById(MOUNT_ID) || (currentScript && currentScript.parentElement) || document.body;
...
(function () {
  ...
  function renderPayload(response) {
    ...
    setStatus("Loaded prototype diagnostic, payload boundary preview, homepage_config, navigation, alerts, email/domain, documents/archive, integrations, deployment/modules, access roles, background_underlay, data-driven cards, page metadata, and admin controls from Supabase payload • " + sortedSections.length + " sections" + (appliedUnderlay && appliedUnderlay.has_managed_asset ? " • managed asset" : "") + (appliedUnderlay && appliedUnderlay.frontend_preload ? " • preload ready" : "") + (appliedMetadata ? " • browser title" : "") + (appliedUnderlay && appliedUnderlay.transform_applied ? " • transformed image" : ""));
  }
...
/* syncetc_current_public_preview_mode_toggle_v13_runtime - BEGIN */
(function syncetcInstallPreviewModeToggle() {
  const ROOT_ID = "syncetc-generated-homepage-v2";
  const STORAGE_KEY = "syncetc_preview_mode_v13";

  function install() {
    const root = document.getElementById(ROOT_ID);
    if (!root) return;

    const topbar = root.querySelector(".se-topbar");
    const existingCustomerSwitcher = root.querySelector(".se-switcher");
    if (!topbar || root.querySelector("[data-se-preview-mode-switcher]")) return;

    const modeSwitcher = document.createElement("div");
    modeSwitcher.className = "se-mode-switcher";
    modeSwitcher.setAttribute("data-se-preview-mode-switcher", "true");
    modeSwitcher.setAttribute("aria-label", "Prototype view mode switcher");
    modeSwitcher.innerHTML =
      '<button type="button" data-se-preview-mode="prototype">Prototype</button>' +
      '<button type="button" data-se-preview-mode="public">Public preview</button>';

    if (existingCustomerSwitcher && existingCustomerSwitcher.parentElement === topbar) {
      existingCustomerSwitcher.insertAdjacentElement("afterend", modeSwitcher);
    } else {
      topbar.appendChild(modeSwitcher);
    }

    const buttons = Array.from(modeSwitcher.querySelectorAll("[data-se-preview-mode]"));

    function setMode(mode) {
      const normalized = mode === "public" ? "public" : "prototype";
      root.setAttribute("data-se-preview-mode", normalized);
      root.classList.toggle("se-public-preview-mode", normalized === "public");
      buttons.forEach(function (button) {
        button.classList.toggle("is-active", button.getAttribute("data-se-preview-mode") === normalized);
      });
      try {
    ...
    buttons.forEach(function (button) {
      button.addEventListener("click", function () {
        setMode(button.getAttribute("data-se-preview-mode"));
      });
    });

    let saved = "prototype";
    try {
      saved = window.localStorage.getItem(STORAGE_KEY) || "prototype";
    } catch (error) {}
    setMode(saved);
  }
...
/* syncetc_current_public_preview_mode_toggle_v13_runtime - END */
