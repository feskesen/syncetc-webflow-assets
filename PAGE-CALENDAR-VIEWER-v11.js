/* PAGE-CALENDAR-VIEWER-v11.js - BEGIN */
(function(){
"use strict";
...
function ensureComponents(){var base=componentBaseUrl();return COMPONENT_FILES.reduce(function(p,file){return p.then(function(){return loadScriptOnce(base + file + "?v=calendar-viewer-views-delay-v11");});},Promise.resolve());}
var VERSION="PAGE-CALENDAR-VIEWER-v11",EDGE_READ_URL="https://ocdaohkiwonjmirqkjww.supabase.co/functions/v1/syncetc-calendar-viewer-read";
var state={shell:null,events:[],visibleEvents:[],selectedEvent:null,viewMode:"compact",currentMonth:null,dateFilter:"upcoming",typeFilter:"all",search:"",loading:true,error:"",lastCustomerKey:"",refreshTimer:null,refreshBusy:false,refreshQueued:false,startupComplete:false,startupFrozen:true,firstRenderDone:false,initialFetchDone:false,initialFallbackTimer:null,lastRefreshAt:0};
...
.cv-month-event:hover{filter:brightness(.97)}
...
@media(max-width:780px){.cv-modal-body{grid-template-columns:1fr}.cv-modal-head{padding:20px}.cv-modal-title{font-size:24px}
.cv-view-row{display:flex;flex-wrap:wrap;gap:8px;align-items:center;justify-content:space-between;padding:14px 30px;background:rgba(255,255,255,.92);border-bottom:1px solid var(--se-aero-border)}
.cv-view-buttons{display:flex;flex-wrap:wrap;gap:8px}
.cv-month-nav{display:flex;align-items:center;gap:8px}
.cv-month-title{min-width:170px;text-align:center;color:var(--se-aero-navy-dark);font-size:13px;font-weight:950;text-transform:uppercase;letter-spacing:.04em}
...
.cv-expanded-card .cv-thumb img,.cv-expanded-card .cv-img-ph{max-width:170px;max-height:160px;width:170px;height:160px}
@media(max-width:980px){.cv-expanded-card{grid-template-columns:112px 1fr}.cv-expanded-card .cv-thumb{grid-column:1/-1;border-left:0;border-top:1px solid rgba(18,54,90,.10)}.cv-month-grid{grid-template-columns:1fr}.cv-month-dow{display:none}.cv-month-cell{min-height:auto}.cv-month-cell.out{display:none}}
.cv-modal-backdrop{padding:14px}.cv-modal-side{order:-1}}
...
function renderEvents(){var status=byId("cv-status");if(status){status.textContent=state.error?"Calendar unavailable.":state.loading?"Loading events...":state.visibleEvents.length+" visible event"+(state.visibleEvents.length===1?"":"s")+".";}var grid=byId("cv-grid");if(!grid)return;if(state.error){grid.className="cv-grid";grid.innerHTML='<div class="cv-error">'+esc(state.error)+'</div>';return;}if(state.loading){grid.className="cv-grid";grid.innerHTML='<div class="cv-empty">Loading events...</div>';return;}if(!state.visibleEvents.length){grid.className="cv-grid";grid.innerHTML='<div class="cv-empty">No events match the current filters.</div>';return;}if(state.viewMode==="month"){renderMonthView(grid);return;}if(state.viewMode==="expanded"){grid.className="cv-expanded-grid";grid.innerHTML=state.visibleEvents.map(expandedCard).join("");return;}grid.className="cv-grid";grid.innerHTML=state.visibleEvents.map(card).join("");}
...
/* PAGE-CALENDAR-VIEWER-v11.js - END */
