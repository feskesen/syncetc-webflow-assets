...
/* PAGE-DESIGN-STUDIO-v50.js | auto-load data bridge without JWT dependency | Generated: 2026-06-01 */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v50";
...
var STYLE_ID="syncetc-design-studio-v50-style";
...
function applyPageReadBridgePayload(payload){
  ...
  if(pageKey==="assets"){
    state.data.records.assets=payload.records||[];
    state.data.assets=payload.records||[];
    state.assetRecords=payload.records||[];
    state.aircraftRecords=payload.records||[];
  }else if(pageKey==="info"){
    state.data.records.info=payload.records||{};
    state.data.info_records=payload.records||{};
    state.infoRecords=payload.records||{};
    if(payload.records&&payload.records.faqs)state.faqs=payload.records.faqs;
    if(payload.records&&payload.records.officers)state.officers=payload.records.officers;
  }

  if(payload.page_settings){
    state.sideAppliedValues=Object.assign({},state.sideAppliedValues||{},payload.page_settings);
    state.sideDraftValues=Object.assign({},state.sideDraftValues||{},payload.page_settings);
  }
}
...
function realPageValuesFor(prefix){
  ...
  vals=Object.assign({},bridgeSettings,vals);
  var out={};
  Object.keys(vals||{}).forEach(function(k){
    if((k.indexOf(prefix+".")===0 || k.indexOf(prefix+"_")===0 || (prefix==="assets"&&k.indexOf("aircraft.")===0)) && clean(vals[k]))out[k]=vals[k];
  });
  return out;
}
...
function hasConnectedRecordData(kind){
  ...
  var candidates=[];
  if(kind==="assets" && Array.isArray(bridgeRecords) && bridgeRecords.length)return true;
  if(kind==="info" && bridgeRecords && typeof bridgeRecords==="object" && (arr(bridgeRecords.faqs).length||arr(bridgeRecords.officers).length||Object.keys(bridgeRecords).length))return true;
  if(kind==="assets"){
    candidates=[
      d.assets,d.asset_records,d.aircraft,d.aircraft_records,d.customer_assets,d.customer_aircraft,
      d.records&&d.records.assets,d.records&&d.records.aircraft,
      state.assets,state.aircraft,state.assetRecords,state.aircraftRecords
    ];
  }else if(kind==="info"){
    candidates=[
      d.faqs,d.info_faqs,d.customer_faqs,d.officers,d.board_members,d.info_records,
      d.records&&d.records.faqs,d.records&&d.records.officers,
      state.faqs,state.officers,state.infoRecords
    ];
  }
