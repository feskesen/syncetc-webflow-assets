/* PAGE-DESIGN-STUDIO-v3.js | sticky layered design studio with effect overrides | Generated: 2026-06-01 03:05:00 UTC */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v3";
...
var STYLE_ID="syncetc-design-studio-v3-style";
var CONTROL_ORDER=["customer","layout","style","typography","color","effect","shadow","border","corner","gradient"];
...
function cloneSelected(v){return {layout:v.layout,style:v.style,typography:v.typography,color:v.color,effect:v.effect,shadow:v.shadow,border:v.border,corner:v.corner,gradient:v.gradient};}
...
function installStyles(){
  ...
  style.textContent='\
    ...
    #'+ROOT_ID+'{--ds-shell-max:1240px;--ds-sticky-top:0px;--ds-control-h:202px;--ds-primary:#12365a;--ds-accent:#2f80c4;--ds-soft:#eaf5ff;--ds-text:#17212b;--ds-muted:#5d6b78;--ds-bg:#fff;--ds-font:Arial,Helvetica,sans-serif;--ds-radius:22px;--ds-button-radius:999px;--ds-eyebrow-radius:999px;--ds-gap:18px;--ds-shadow:0 10px 28px rgba(12,38,64,.10);--ds-border:1px solid rgba(18,54,90,.14);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;color:#17212b;min-height:100vh;background:linear-gradient(180deg,#f7fbff 0%,#ffffff 220px);}\
    ...
    #'+ROOT_ID+' .ds-effect-grid{display:grid;grid-template-columns:1.18fr repeat(4,minmax(128px,1fr));gap:10px;align-items:end;margin-top:10px;padding-top:10px;border-top:1px solid rgba(18,54,90,.10);}\
    ...
    @media(max-width:1080px){#'+ROOT_ID+' .ds-toolbar-grid,#'+ROOT_ID+' .ds-effect-grid{grid-template-columns:1fr 1fr 1fr;}#'+ROOT_ID+'{--ds-control-h:304px;}}\
    @media(max-width:760px){#'+ROOT_ID+' .ds-shell{padding:18px 14px 48px;}#'+ROOT_ID+' .ds-hero-top{display:block;}#'+ROOT_ID+' .ds-version{margin-top:12px;}#'+ROOT_ID+' .ds-toolbar-grid,#'+ROOT_ID+' .ds-effect-grid{grid-template-columns:1fr;}#'+ROOT_ID+'{--ds-control-h:700px;}#'+ROOT_ID+' .ds-toolbar-actions{display:block;}#'+ROOT_ID+' .ds-actions{justify-content:flex-start;margin-top:10px;}#'+ROOT_ID+' .ds-preview-head{display:block;}#'+ROOT_ID+' .ds-chip-row{justify-content:flex-start;margin-top:10px;}#'+ROOT_ID+' .ds-preview-header{grid-template-columns:1fr;}#'+ROOT_ID+' .ds-grid{grid-template-columns:1fr!important;}}\
  ';
  ...
  mount.innerHTML='<div id="'+ROOT_ID+'" class="'+(state.loading?'ds-skeleton':'')+'">'+
    '<div class="ds-shell">'+
      ...
      '<div class="ds-toolbar-wrap"><section class="ds-toolbar" aria-label="Design Studio Controls">'+
        ...
        '</div>'+
        '<div class="ds-toolbar-actions"><div class="ds-status" data-ds-status></div><div class="ds-actions"><button class="ds-btn" type="button" data-ds-action="save">Save to customer</button><button class="ds-btn secondary" type="button" data-ds-action="save-profile" disabled>Save as design profile</button><button class="ds-btn secondary" type="button" data-ds-action="reset">Reset preview</button><button class="ds-btn secondary" type="button" data-ds-action="reload">Reload</button></div></div>'+
      '</section></div>'+
    ...
    '<aside class="ds-side-drawer" data-ds-side-drawer><div class="ds-side-head"><div><h3>Page text loader</h3><p>Placeholder for customer/page content editing. Not wired in this design-only version.</p></div><button class="ds-side-close" type="button" data-ds-action="side-close">×</button></div><div class="ds-side-body"><div class="ds-side-disabled">This drawer will later list enabled pages for the selected customer and expose editable page text fields. v3 intentionally leaves it disabled until the content-editing schema path is audited.</div></div></aside>'+
  '</div>';
  state.booted=true;
  bindFrame();
  hydrateControls();
...
function updatePreview(immediate){
  ...
  var run=function(){
    var vars=cssVars();
    var chipHtml='<span class="ds-chip">'+esc(displayPresetLabel(state.selected.layout,state.selected.layout))+'</span><span class="ds-chip">'+esc(displayPresetLabel(state.selected.style,state.selected.style))+'</span><span class="ds-chip">'+esc(displayPresetLabel(state.selected.typography,state.selected.typography))+'</span><span class="ds-chip">'+esc(state.selected.color)+'</span><span class="ds-chip">'+esc(state.selected.effect)+'</span><span class="ds-chip">shadow: '+esc(state.selected.shadow)+'</span><span class="ds-chip">border: '+esc(state.selected.border)+'</span><span class="ds-chip">corner: '+esc(state.selected.corner)+'</span><span class="ds-chip">gradient: '+esc(state.selected.gradient)+'</span>';
...
function bindFrame(){
  ...
  r.querySelectorAll('[data-ds-action]').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      if(action==='save-profile')return;
      if(action==='side-open'){var d=r.querySelector('[data-ds-side-drawer]');if(d)d.classList.add('is-open');return;}
      if(action==='side-close'){var c=r.querySelector('[data-ds-side-drawer]');if(c)c.classList.remove('is-open');return;}
