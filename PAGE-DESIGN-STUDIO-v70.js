...
/* PAGE-DESIGN-STUDIO-v70.js | discard unapplied drawer edits only | Generated: 2026-06-02 */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v70";
...
var STYLE_ID="syncetc-design-studio-v70-style";
...
/* v70 discard-unapplied-only helpers - BEGIN */
function baseValuesForDiscardUnappliedEdits(){
  if(objectHasKeys(state.sideAppliedValues))return cloneObj(state.sideAppliedValues);
  if(objectHasKeys(state.sideLoadedValues))return cloneObj(state.sideLoadedValues);
  return cloneObj(loadedValuesForCurrentSidePage());
}

function discardUnappliedDrawerEdits(){
  var base=baseValuesForDiscardUnappliedEdits();
  state.sideDraftValues=cloneObj(base);
  state.sideDirty=false;
  state.sideSaving=false;
  state.error="";
  state.notice=state.sideAppliedDirty
    ? "Unapplied drawer edits discarded. Applied preview changes are still pending save."
    : "Unapplied drawer edits discarded.";
}

function setVisiblePageFieldValues(values){
  var r=root();
  if(!r||!values)return;
  r.querySelectorAll('[data-ds-page-field]').forEach(function(el){
    var key=el.getAttribute('data-ds-page-field');
    if(!key)return;
...
function afterDiscardUnappliedRender(){
  var values=cloneObj(state.sideDraftValues||{});
  renderFrame();
  setTimeout(function(){
    setVisiblePageFieldValues(values);
    syncSideActionButtonsDom();
    updateStatus();
  },0);
}
/* v70 discard-unapplied-only helpers - END */
    ...
    '<aside class="ds-side-drawer '+(state.sideOpen?'is-open':'')+'" data-ds-side-drawer>'+
      ...
      '<div class="ds-side-body">'+
        ...
        '<div class="ds-side-actions">'+
          ...
          '<button class="ds-btn secondary" type="button" data-ds-action="discard-page-text" '+(!state.sideDirty||state.sideSaving?'disabled':'')+'>Discard Changes</button>'+
        '</div>'+
...
function collectPageTextValues(){
  var out=cloneObj(state.sideDraftValues||sideDrawerDisplayValues()||{});
  var r=root();
  if(!r)return out;
  r.querySelectorAll('[data-ds-page-field]').forEach(function(el){
...
function discardPageText(doRender){
  discardUnappliedDrawerEdits();
  if(doRender!==false)afterDiscardUnappliedRender();
}
...
function bindFrame(){
  ...
  r.querySelectorAll('[data-ds-page-field]').forEach(function(el){
    el.addEventListener('input',function(){
      ...
      if(!state.sideDraftValues)state.sideDraftValues=cloneObj(sideDrawerDisplayValues());
...
/* v70 discard semantics diagnostic - BEGIN */
window.SYNCETC_V70_DISCARD_STATE=function(){
  var out={
    version:VERSION,
    sideDirty:state.sideDirty,
    sideAppliedDirty:state.sideAppliedDirty,
    ...
    appliedHero:state.sideAppliedValues&&state.sideAppliedValues["aircraft.heroEyebrow"],
    drawerHero:(function(){
      var el=root()&&root().querySelector('[data-ds-page-field="aircraft.heroEyebrow"]');
      return el?el.value:null;
    })(),
    previewHero:livePageTextValues&&livePageTextValues()["aircraft.heroEyebrow"]
  };
  console.log("SYNCETC_V70_DISCARD_STATE",out);
  return out;
};
/* v70 discard semantics diagnostic - END */

installUnsavedGuardsOnce();
/* PAGE-DESIGN-STUDIO-v70.js - END */
