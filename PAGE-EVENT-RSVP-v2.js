/* PAGE-EVENT-RSVP-v2.js - BEGIN */
(function(){
"use strict";

var VERSION="PAGE-EVENT-RSVP-v2";
...
function scriptBase(){
  ...
  for(var i=scripts.length-1;i>=0;i--){
    ...
    if(src.indexOf("PAGE-EVENT-RSVP-v2.js")>-1){
...
function loadComponents(){
  ...
  COMPONENT_FILES.forEach(function(file){
    p=p.then(function(){return loadScript(base+file+"?v=event-rsvp-v2");});
  });
  return p;
}
...
function tokenFromStorage(){
  function findAccessToken(obj,depth){
    ...
    for(var i=0;i<keys.length;i++){
      var found=findAccessToken(obj[keys[i]],depth+1);
  ...
  try{
    for(var i=0;i<localStorage.length;i++){
      ...
      if(low.indexOf("auth")>=0||low.indexOf("sb-")===0||low.indexOf("supabase")>=0){
        var found=findAccessToken(localStorage.getItem(key),0);
        if(found)return found;
      }
    }
  }catch(e){}
  ...
  try{
    for(var j=0;j<sessionStorage.length;j++){
      ...
      var slow=skey.toLowerCase();
      if(slow.indexOf("auth")>=0||slow.indexOf("sb-")===0||slow.indexOf("supabase")>=0){
        var sfound=findAccessToken(sessionStorage.getItem(skey),0);
...
function getAuthToken(){
  ...
  try{
    if(C().SecurityContext&&C().SecurityContext.getToken){
      token=C().SecurityContext.getToken()||"";
      if(token){setUserFromToken(token);return Promise.resolve(token);}
    }
  }catch(e){}
  ...
  try{
    if(C().AuthContext&&C().AuthContext.getSnapshot){
      var snap=C().AuthContext.getSnapshot()||{};
      if(snap.user)state.user=snap.user;
    }
  }catch(e){}
  ...
  try{
    if(C().AuthContext&&C().AuthContext.init){
      return Promise.resolve(C().AuthContext.init({activeCustomerKey:state.customerKey}))
        .then(function(){
          var t="";
          try{if(C().SecurityContext&&C().SecurityContext.getToken)t=C().SecurityContext.getToken()||"";}catch(e){}
          if(!t){try{if(C().AuthContext&&C().AuthContext.getToken)t=C().AuthContext.getToken()||"";}catch(e){}}
          if(!t)t=tokenFromStorage();
          if(t)setUserFromToken(t);
          return t;
        })
        .catch(function(){
          var fallback=tokenFromStorage();
          if(fallback)setUserFromToken(fallback);
          return fallback;
        });
    }
  }catch(e){}
...
function callRsvpFunction(payload){
  return getAuthToken().then(function(token){
    ...
    if(!state.authToken){
      throw new Error("You must be logged in before using RSVP. If you are already logged in, click Refresh after the login state finishes loading.");
    }
...
function styleHtml(){
  return '<style id="PAGE-EVENT-RSVP-v2-style">'+
...
function messageHtml(){
  ...
  return '<div class="syncetc-rsvp-alert'+cls+'">'+esc(state.message)+'</div>';
}
