...
(function(){
  ...
  var VERSION = "PAGE-EVENT-ADMIN-v2";
  ...
  function installStyles(){
    U().installStyle("PAGE-EVENT-ADMIN-v2-style", `
      ...
      .ea-disabled-note { color:#8a1f1f; font-size:12px; font-weight:800; margin-top:-6px; }
  ...
  function renderCreateTab(){
    ...
    return '<section class="ea-panel"><div class="ea-toolbar"><div><h2>'+(isEdit?'Edit Event':'Create Event')+'</h2><p>Use a blank event, a saved event type/location, or preselect from a prior event.</p></div><div class="ea-toolbar-right"><button type="button" class="ea-btn" data-ea-clear-form>Blank Event</button><button type="button" class="ea-btn primary" data-ea-save-event>'+(isEdit?'Save Changes':'Create Event')+'</button></div></div>'+ 
      ...
      '<div class="ea-grid two"><label class="ea-field"><span>New Event Type Name</span><input class="ea-input" id="ea-new-type-name" placeholder="Only if this is a new type"></label><div class="ea-field"><span>Save Type</span><button type="button" class="ea-btn" data-ea-save-new-type-from-event>Save as New Event Type</button></div></div>'+ 
      '<div class="ea-grid three"><label class="ea-field"><span>Start</span><input class="ea-input" id="ea-start" type="datetime-local" step="300" value="'+esc(toDatetimeLocal(ev.start_at))+'"></label><label class="ea-field"><span>End</span><input class="ea-input" id="ea-end" type="datetime-local" step="300" value="'+esc(toDatetimeLocal(ev.end_at))+'"></label><label class="ea-field"><span>Date note</span><input class="ea-input" id="ea-date-note" value="'+esc(ev.date_note)+'"><div class="ea-disabled-note">Date notes display in the alternate/accent color.</div></label></div>'+ 
      ...
      '<label class="ea-field"><span>Address</span><input class="ea-input" id="ea-address" value="'+esc(ev.address)+'"></label><div class="ea-inline-tools"><button type="button" class="ea-btn" data-ea-event-map-from-address>Get Google Maps URL from location/address</button><button type="button" class="ea-btn" data-ea-save-new-location-from-event>Save as New Location</button></div>'+ 
  ...
  function updateTimeFieldState(){
    ...
    var noEnd = checked("ea-no-end-time");
    var start = byId("ea-start");
    var end = byId("ea-end");
    if(start) start.disabled = allDay;
    if(end) end.disabled = allDay || noEnd;
  }
