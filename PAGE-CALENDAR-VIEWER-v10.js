/* PAGE-CALENDAR-VIEWER-v10.js - BEGIN */
(function(){
"use strict";
...
function ensureComponents(){var base=componentBaseUrl();return COMPONENT_FILES.reduce(function(p,file){return p.then(function(){return loadScriptOnce(base + file + "?v=calendar-viewer-render-fix-v10");});},Promise.resolve());}
var VERSION="PAGE-CALENDAR-VIEWER-v10",EDGE_READ_URL="https://ocdaohkiwonjmirqkjww.supabase.co/functions/v1/syncetc-calendar-viewer-read";
var state={shell:null,events:[],visibleEvents:[],selectedEvent:null,dateFilter:"upcoming",typeFilter:"all",search:"",loading:true,error:"",loadedEver:false,lastCustomerKey:"",refreshTimer:null,refreshBusy:false,refreshQueued:false,startupComplete:false,startupFrozen:true,firstRenderDone:false,initialFetchDone:false,initialFallbackTimer:null,lastRefreshAt:0,stableCustomerTimer:null};
...
function renderEvents(){var status=byId("cv-status");if(status){status.textContent=state.error?"Calendar unavailable.":state.loading?"Loading events...":state.visibleEvents.length+" visible event"+(state.visibleEvents.length===1?"":"s")+".";}var grid=byId("cv-grid");if(!grid)return;if(state.error){grid.innerHTML='<div class="cv-error">'+esc(state.error)+'</div>';return;}if(state.loading&&!state.loadedEver){grid.innerHTML='<div class="cv-empty">Loading events...</div>';return;}grid.innerHTML=state.visibleEvents.length?state.visibleEvents.map(card).join(""):'<div class="cv-empty">No events match the current filters.</div>';}
...
function forceCalendarRefresh(){state.loading=false;installStyles();createFilters();applyFilters();renderEvents();var search=byId("cv-search");if(search)search.value=state.search;if(state.selectedEvent)renderModal();}
  ...
  Promise.resolve()
    ...
    .then(function(){
      ...
      if(reason==="customer-hard-change" && afterKey!==state.lastCustomerKey){
        ...
        return fetchData().then(function(){rerender();forceCalendarRefresh();});
      }
      state.lastCustomerKey=afterKey||beforeKey||state.lastCustomerKey;
      return fetchData().then(forceCalendarRefresh);
    })
    .catch(function(){
      renderCalendarOnly();
    })
...
/* PAGE-CALENDAR-VIEWER-v10.js - END */
