/* PAGE-GALLERY-ADMIN-v1.js - BEGIN */
(function () {
  "use strict";

  var VERSION="PAGE-GALLERY-ADMIN-v1";
  ...
  function renderHero() {
    ...
    return '<section class="se-ga-hero">' +
      '<div class="se-ga-eyebrow">Admin Tool · ' + esc(activeCustomerLabel()) + '</div>' +
  ...
  function renderPage() {
    installStyles();
    var bodyHtml = "";
    if (!window.SyncEtc || !window.SyncEtc.AuthContext || !window.SyncEtc.AuthContext.getSnapshot().signed_in) {
      bodyHtml = '<main class="se-ga-main"><div class="se-ga-empty"><strong>Sign in required.</strong><br>Use the header Login button to access Gallery Management.</div></main>';
    } else if (!canManageGallery()) {
      bodyHtml = '<main class="se-ga-main"><div class="se-ga-empty"><strong>Access denied.</strong><br>This account does not have Gallery Admin management access for the active customer.</div></main>';
    } else {
      bodyHtml = '<main class="se-ga-main">' +
        renderTabs() +
        renderQuality() +
    ...
    var html = '<div class="se-ga-page"><div class="se-ga-shell">' + renderHero() + bodyHtml + '</div></div>' + renderModal() + renderConfirm();
  ...
  function init() {
    ensureComponents().then(function () {
      ...
      if (window.SyncEtc && window.SyncEtc.AuthContext) {
        ...
        return window.SyncEtc.AuthContext.init().then(function () {
          if (canManageGallery()) return loadItems();
          state.loading = false;
          renderPage();
        });
      }
