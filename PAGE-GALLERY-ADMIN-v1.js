...
(function () {
  ...
  function restHeaders() {
    var key = window.SYNCETC_SUPABASE_ANON_KEY || window.SUPABASE_ANON_KEY || "";
    if (!key) return null;
  ...
  function callEdge(action, payload) {
    var h = restHeaders();
    if (!h) return Promise.reject(new Error("Missing Supabase publishable/anon key."));
  ...
  function installStyles() {
    U().installStyle("PAGE-GALLERY-ADMIN-v1-style", `
