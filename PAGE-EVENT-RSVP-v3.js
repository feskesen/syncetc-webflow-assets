/* PAGE-EVENT-RSVP-v3.js - BEGIN */
(function(){
"use strict";

var VERSION="PAGE-EVENT-RSVP-v3";
...
function scriptBase(){
  ...
  for(var i=scripts.length-1;i>=0;i--){
    ...
    if(src.indexOf("PAGE-EVENT-RSVP-v3.js")>-1){
...
function loadComponents(){
  ...
  COMPONENT_FILES.forEach(function(file){
    p=p.then(function(){return loadScript(base+file+"?v=event-rsvp-v3");});
  });
  return p;
}
...
function callRsvpFunction(payload){
  return getAuthToken().then(function(token){
    ...
    if(!state.authToken||!isLikelySupabaseJwt(state.authToken)){
      throw new Error("No valid Supabase login token was found for RSVP. Log in again on this page, then reload and try RSVP.");
    }
    ...
    return resp.text().then(function(txt){
      ...
      if(!resp.ok||data.ok===false){
        if(resp.status===401){
          throw new Error("RSVP request failed: 401. The page reached Supabase, but Supabase did not accept the current login token.");
        }
...
function styleHtml(){
  return '<style id="PAGE-EVENT-RSVP-v3-style">'+
