...
/* PAGE-DESIGN-STUDIO-v69.js | discard legacy alias compatibility fix | Generated: 2026-06-02 */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v69";
...
var STYLE_ID="syncetc-design-studio-v69-style";
...
var SYNCETC_CUSTOMER_REGISTRY={
  "150th-aero":{
    ...
    legacy_customer_names:["150th Aero Flying Club","demo_flying_club","150th_aero","150th-aero","150thaero"]
  },
  "test-customer":{
...
/* v66 force drawer DOM sync after state changes - BEGIN */
function setControlValueIfPresent(el,value){
  if(!el)return;
...
function syncDrawerDomFromStateValues(values){
  var r=root();
  if(!r||!values)return;
  Object.keys(values).forEach(function(key){
    var value=values[key];
    var selectors=[
      '[data-ds-page-field="'+key+'"]',
      '[data-field-key="'+key+'"]',
    ...
    selectors.forEach(function(sel){
      r.querySelectorAll(sel).forEach(function(el){
        setControlValueIfPresent(el,value);
      });
    });
  });

  var pageAccess=values.page_access_scope||state.pageAccessScope;
  r.querySelectorAll('[data-ds-page-field="page_access_scope"],[data-field-key="page_access_scope"],[data-setting-key="page_access_scope"],[data-key="page_access_scope"],[name="page_access_scope"]').forEach(function(el){
    setControlValueIfPresent(el,pageAccess);
  });
}

function afterDiscardSyncDrawerAndPreview(){
  var loaded=cloneObj(state.sideLoadedValues||{});
  ...
  state.error="";
  state.notice="Unsaved drawer changes discarded.";
  renderFrame();
  setTimeout(function(){
    syncDrawerDomFromStateValues(state.sideDraftValues);
    syncSideActionButtonsDom();
    updateStatus();
  },0);
}
/* v66 force drawer DOM sync after state changes - END */

/* v65 discard snapshot fix - BEGIN */
function pageSelectionSignature(){
  var customer="";
  try{
    var ctx=activeCustomerContext&&activeCustomerContext();
    customer=(ctx&&ctx.customer_key)||state.customer_key||state.customerKey||"";
  }catch(_err){
    customer=state.customer_key||state.customerKey||"";
  }
  return [clean(customer),clean(selectedPageKey&&selectedPageKey()),clean(selectedPageLabel&&selectedPageLabel())].join("||");
}

function ensureLoadedSnapshotForCurrentSelection(){
  var sig=pageSelectionSignature();
  if(state.sideLoadedSignature===sig&&objectHasKeys(state.sideLoadedValues))return;
  var base=loadedValuesForCurrentSidePage();
  state.sideLoadedSignature=sig;
...
function restoreLoadedSnapshotForDiscard(){
  ensureLoadedSnapshotForCurrentSelection();
  var loaded=cloneObj(state.sideLoadedValues||{});
  state.sideDraftValues=cloneObj(loaded);
  state.sideAppliedValues={};
  state.sideDirty=false;
...
function sideDrawerDisplayValues(){
  ...
  return loadedValuesForCurrentSidePage ? loadedValuesForCurrentSidePage() : {};
}
...
function initSideEditState(reset){
  ...
  var sig=pageSelectionSignature();
  if(!reset&&state.sideLoadedSignature===sig&&objectHasKeys(state.sideLoadedValues)&&objectHasKeys(state.sideDraftValues))return;
...
function collectPageTextValues(){
  var r=root();
  var out=cloneObj(state.sideDraftValues||{});
  if(!r)return out;
  r.querySelectorAll('[data-ds-page-field],[data-field-key],[data-setting-key],[data-key]').forEach(function(el){
    var key=el.getAttribute('data-ds-page-field')||el.getAttribute('data-field-key')||el.getAttribute('data-setting-key')||el.getAttribute('data-key');
    if(!key)return;
...
function applySavedPageSettingsLocally(values,saved,key,label,fields){
  ...
  state.sideDirty=false;
  state.sideAppliedDirty=false;
  state.sideLoadedSignature=pageSelectionSignature();
  state.sideLoadedValues=cloneObj(values);
  state.sideDraftValues=cloneObj(values);
  state.sideAppliedValues={};
  state.error="";
  state.notice="Page settings saved for "+(saveKey==="aircraft"?"Assets":label)+".";
}
...
function v63StrictDiscardPageText(){
  v68DiscardPageSpecificChanges();
}
...
function savePageText(){
  ...
  var initialKey=currentPreviewPageKeyForSave();
  ...
  var values=saveValuesFromAppliedState();
  var key=canonicalPageKeyForSettingsSave(initialKey,values,initialLabel);
  var row=selectedPageRowForSettingsSave(values);
  var label=(key==="aircraft"?"Assets":(initialLabel||selectedPageLabel()));
  var fields=normalizePageFields(row,key);
...
function bindFrame(){
  ...
  r.querySelectorAll('[data-ds-action]').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      if(action==='side-close'){if(hasSideUnsaved()&&!window.confirm('Discard unapplied/unsaved Page Text changes and close?'))return;state.sideOpen=false;state.sideDirty=false;state.sideAppliedDirty=false;state.sideDraftValues=null;state.sideAppliedValues=null;state.notice='Page Specific Settings closed. Dropdown-backed inheritance audit active.';renderFrame();return;}
...
/* v57 short unsaved-guard diagnostic - BEGIN */
window.SYNCETC_UNSAVED_GUARD_STATE=function(){
  var out={
    version:VERSION,
    sideDirty:state.sideDirty,
    sideAppliedDirty:state.sideAppliedDirty,
    hasUnsaved:hasUnsavedPageSettingChanges(),
    beforeUnloadInstalled:!!window.__SYNCETC_DS_SAFE_BEFOREUNLOAD_V57,
    selectorGuardInstalled:!!window.__SYNCETC_DS_SAFE_SELECTOR_GUARD_V57
  };
  console.log("SYNCETC_UNSAVED_GUARD_STATE",out);
  return out;
};
/* v57 short unsaved-guard diagnostic - END */


/* v58 short save diagnostic - BEGIN */
window.SYNCETC_PAGE_SETTINGS_SAVE_STATE=function(){
  var out={
    version:VERSION,
    ...
    hasToken:!!getToken(),
    customer_key:state.customer_key,
    canonical_customer_key:canonicalCustomerKeyForSave(),
    selected_page_key:selectedPageKey(),
    current_preview_page_key:currentPreviewPageKeyForSave(),
    detected_namespace:detectSettingsNamespace(state.sideAppliedValues||collectPageTextValues()),
    save_page_key:strictCurrentSettingsKey(),
    strict_selected_row_key:(strictSelectedSettingsRowForKey(strictCurrentSettingsKey())||{}).page_key,
    sideDirty:state.sideDirty,
    sideAppliedDirty:state.sideAppliedDirty,
    sideSaving:state.sideSaving
  };
  console.log("SYNCETC_PAGE_SETTINGS_SAVE_STATE",out);
  return out;
};
/* v58 short save diagnostic - END */


/* v63 capture discard fallback - BEGIN */
document.addEventListener("click",function(e){
  var btn=e.target&&e.target.closest?e.target.closest('[data-ds-action="discard-page-text"]'):null;
  if(!btn)return;
  e.preventDefault();
  e.stopImmediatePropagation();
  v63StrictDiscardPageText();
},true);
/* v63 capture discard fallback - END */


/* v63 strict row-selection diagnostic - BEGIN */
window.SYNCETC_PAGE_ROW_SELECTION_STATE=function(){
  var rows=pageRows().map(function(p){
    return {
      customer_key:p.customer_key,
      page_key:p.page_key,
      canonical_page_key:normalizeSettingsPageKeyForCompare(p.page_key),
      page_label:p.page_label,
      is_enabled:p.is_enabled,
      hero_eyebrow:(obj(p.current_settings_json)["aircraft.heroEyebrow"]||obj(p.settings_json)["aircraft.heroEyebrow"]||"")
    };
  });
  var out={
    version:VERSION,
    currentPreviewPageKey:currentPreviewPageKeyForSave(),
    currentPreviewPageLabel:currentPreviewPageLabelForSave(),
    strictCurrentSettingsKey:strictCurrentSettingsKey(),
    strictSelectedRowPageKey:(strictSelectedSettingsRowForKey(strictCurrentSettingsKey())||{}).page_key,
    strictSelectedRowHeroEyebrow:(obj((strictSelectedSettingsRowForKey(strictCurrentSettingsKey())||{}).current_settings_json)["aircraft.heroEyebrow"]||""),
    rows:rows
  };
  console.log("SYNCETC_PAGE_ROW_SELECTION_STATE",out);
  return out;
};
/* v63 strict row-selection diagnostic - END */


/* v64 side drawer state diagnostic - BEGIN */
window.SYNCETC_SIDE_STATE_MACHINE=function(){
  var out={
    version:VERSION,
    selectedPageKey:selectedPageKey(),
    canonicalSidePageKey:currentCanonicalSidePageKey(),
    sideDirty:state.sideDirty,
    sideAppliedDirty:state.sideAppliedDirty,
    sideSaving:state.sideSaving,
    loadedHero:state.sideLoadedValues&&state.sideLoadedValues["aircraft.heroEyebrow"],
    draftHero:state.sideDraftValues&&state.sideDraftValues["aircraft.heroEyebrow"],
    appliedHero:state.sideAppliedValues&&state.sideAppliedValues["aircraft.heroEyebrow"],
    displayHero:sideDrawerDisplayValues()["aircraft.heroEyebrow"],
    previewHero:livePageTextValues()["aircraft.heroEyebrow"]
  };
  console.log("SYNCETC_SIDE_STATE_MACHINE",out);
  return out;
};
/* v64 side drawer state diagnostic - END */


/* v65 capture discard listener - BEGIN */
if(!window.__SYNCETC_DS_DISCARD_CAPTURE_V65){
  window.__SYNCETC_DS_DISCARD_CAPTURE_V65=true;
  document.addEventListener("click",function(e){
    var btn=e.target&&e.target.closest?e.target.closest('[data-ds-action="discard-page-text"]'):null;
    if(!btn)return;
    e.preventDefault();
...
/* v65 capture discard listener - END */


/* v65 discard diagnostic - BEGIN */
window.SYNCETC_DISCARD_SNAPSHOT_STATE=function(){
  var out={
    version:VERSION,
    signature:pageSelectionSignature(),
    loadedSignature:state.sideLoadedSignature,
    loadedHero:state.sideLoadedValues&&state.sideLoadedValues["aircraft.heroEyebrow"],
    draftHero:state.sideDraftValues&&state.sideDraftValues["aircraft.heroEyebrow"],
    appliedHero:state.sideAppliedValues&&state.sideAppliedValues["aircraft.heroEyebrow"],
    sideDirty:state.sideDirty,
  ...
  console.log("SYNCETC_DISCARD_SNAPSHOT_STATE",out);
  return out;
};
/* v65 discard diagnostic - END */


/* v66 drawer/preview sync diagnostic - BEGIN */
window.SYNCETC_DRAWER_PREVIEW_SYNC_STATE=function(){
  var out={
    version:VERSION,
    loadedHero:state.sideLoadedValues&&state.sideLoadedValues["aircraft.heroEyebrow"],
    draftHero:state.sideDraftValues&&state.sideDraftValues["aircraft.heroEyebrow"],
  ...
  console.log("SYNCETC_DRAWER_PREVIEW_SYNC_STATE",out);
  return out;
};
/* v66 drawer/preview sync diagnostic - END */


/* v67 discard field sync diagnostic - BEGIN */
window.SYNCETC_FORCE_DISCARD_SYNC_TEST=function(){
  discardPageText(true);
  setTimeout(function(){
    console.log("SYNCETC_FORCE_DISCARD_SYNC_TEST", window.SYNCETC_DRAWER_PREVIEW_SYNC_STATE ? window.SYNCETC_DRAWER_PREVIEW_SYNC_STATE() : "no sync state helper");
  },25);
};
/* v67 discard field sync diagnostic - END */



/* v68 direct discard implementation - BEGIN */
function v68CurrentDrawerValuesFromLoaded(){
  var loaded=state.sideLoadedValues&&Object.keys(state.sideLoadedValues).length?state.sideLoadedValues:null;
  if(loaded)return cloneObj(loaded);
  if(typeof loadedValuesForCurrentSidePage==="function")return cloneObj(loadedValuesForCurrentSidePage());
  if(typeof strictPageSettingsForKey==="function")return cloneObj(strictPageSettingsForKey("aircraft"));
  return {};
}

function v68SetDrawerFieldValues(values){
  var r=root&&root();
  if(!r||!values)return;
  Object.keys(values).forEach(function(key){
    r.querySelectorAll(
      ...
      else el.value=values[key]==null?"":String(values[key]);
...
function v68DiscardPageSpecificChanges(){
  var values=v68CurrentDrawerValuesFromLoaded();
  state.sideDraftValues=cloneObj(values);
  state.sideAppliedValues={};
  state.sideDirty=false;
  state.sideAppliedDirty=false;
  ...
  renderFrame();
  setTimeout(function(){
    v68SetDrawerFieldValues(values);
    if(typeof syncSideActionButtonsDom==="function")syncSideActionButtonsDom();
    if(typeof updateStatus==="function")updateStatus();
  },0);
}


/* v69 discard legacy alias compatibility - BEGIN */
function v63StrictDiscardPageText(){
  v68DiscardPageSpecificChanges();
}
...
/* v69 discard legacy alias compatibility - END */

if(!window.__SYNCETC_DS_DISCARD_CAPTURE_V68){
  window.__SYNCETC_DS_DISCARD_CAPTURE_V68=true;
  document.addEventListener("click",function(e){
    var btn=e.target&&e.target.closest?e.target.closest('[data-ds-action="discard-page-text"]'):null;
    if(!btn)return;
    e.preventDefault();
...
/* v68 direct discard implementation - END */

/* v68 guard installer safety shim - BEGIN */
if(typeof installUnsavedGuardsOnce!=="function"){
  function installUnsavedGuardsOnce(){
    if(window.__SYNCETC_DS_GUARD_SHIM_V68)return;
    window.__SYNCETC_DS_GUARD_SHIM_V68=true;
    if(typeof hasUnsavedPageSettingChanges==="function"){
      window.addEventListener("beforeunload",function(e){
        if(!hasUnsavedPageSettingChanges())return;
        e.preventDefault();
        e.returnValue="";
...
/* v68 guard installer safety shim - END */


/* v68 boot/discard diagnostic - BEGIN */
window.SYNCETC_V68_STATUS=function(){
  var out={
    version:VERSION,
    hasGuardInstaller:typeof installUnsavedGuardsOnce==="function",
    hasDiscardCapture:!!window.__SYNCETC_DS_DISCARD_CAPTURE_V68,
    loadedHero:state.sideLoadedValues&&state.sideLoadedValues["aircraft.heroEyebrow"],
    draftHero:state.sideDraftValues&&state.sideDraftValues["aircraft.heroEyebrow"],
    appliedHero:state.sideAppliedValues&&state.sideAppliedValues["aircraft.heroEyebrow"],
    sideDirty:state.sideDirty,
  ...
  console.log("SYNCETC_V68_STATUS",out);
  return out;
};
/* v68 boot/discard diagnostic - END */


/* v69 discard alias diagnostic - BEGIN */
window.SYNCETC_V69_DISCARD_ALIAS_STATE=function(){
  var out={
    version:VERSION,
    hasV68Discard:typeof v68DiscardPageSpecificChanges==="function",
    hasLegacyUpper:typeof v63StrictDiscardPageText==="function",
    hasLegacyLower:typeof v63StrictdiscardPageText==="function",
    hasDiscardPageText:typeof discardPageText==="function",
    loadedHero:state.sideLoadedValues&&state.sideLoadedValues["aircraft.heroEyebrow"],
    draftHero:state.sideDraftValues&&state.sideDraftValues["aircraft.heroEyebrow"],
    appliedHero:state.sideAppliedValues&&state.sideAppliedValues["aircraft.heroEyebrow"]
  };
  console.log("SYNCETC_V69_DISCARD_ALIAS_STATE",out);
  return out;
};
/* v69 discard alias diagnostic - END */

installUnsavedGuardsOnce();
/* PAGE-DESIGN-STUDIO-v69.js - END */
