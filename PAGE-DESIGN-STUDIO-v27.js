/* PAGE-DESIGN-STUDIO-v27.js | actual aircraft page preview wiring | Generated: 2026-06-01 14:35:00 UTC */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v27";
...
var STYLE_ID="syncetc-design-studio-v27-style";
...
/* v27 actual aircraft page CSS from PAGE-AIRCRAFT-v2 */
...
/* v27 actual aircraft page CSS end */
...
function livePageTextValues(){
  ...
  var current=state.sideOpen?collectPageTextValues():{};
...
function fieldValue(settings,key,fallback){
  settings=settings||{};
  var aliases={
    hero_eyebrow:["hero_eyebrow","heroEyebrow","eyebrow","hero eyebrow"],
    hero_title:["hero_title","heroTitle","page_title","pageTitle","title","hero title"],
    hero_intro:["hero_intro","heroIntro","intro_text","introText","description","hero intro"],
    bubble_1_value:["bubble_1_value","bubble1Value","bubble 1 value"],
    bubble_1_text:["bubble_1_text","bubble1Text","bubble 1 text"],
    bubble_2_value:["bubble_2_value","bubble2Value","bubble 2 value"],
    bubble_2_text:["bubble_2_text","bubble2Text","bubble 2 text"],
    bubble_3_value:["bubble_3_value","bubble3Value","bubble 3 value"],
    bubble_3_text:["bubble_3_text","bubble3Text","bubble 3 text"],
    intro_label:["intro_label","introLabel","section_label","sectionLabel"],
    intro_title:["intro_title","introTitle","section_title","sectionTitle"],
    intro_text:["intro_text","introText","section_text","sectionText"],
    bottom_note:["bottom_note","bottomNote","note","footer_note","footerNote"]
  };
...
function actualAircraftDefaults(){
  return {
    heroEyebrow:"Aircraft Fleet",
    heroTitle:"Our Aircraft",
    heroIntro:"The 150th Aero Flying Club maintains a practical fleet of Cessna aircraft for qualified members.",
    stats:[
      {value:"FOUR",text:"Club aircraft currently available to qualified members."},
      {value:"KMMU / KSMQ",text:"Aircraft are based at Morristown Municipal Airport and Somerset Airport."},
      {value:"C172",text:"A practical training and travel platform familiar to many general aviation pilots."}
    ],
    introLabel:"Fleet Overview",
    introTitle:"Well-equipped aircraft for Club flying",
    introText:"Each aircraft has its own equipment profile and avionics package. Current availability, rates, squawks, and operating limitations should be confirmed through Club systems and current Club rules.",
    note:"Aircraft information shown on this page is for general orientation only.",
    aircraft:[]
  };
}

function aircraftPreviewConfigFromPageText(){
  var settings=livePageTextValues();
  var cfg=actualAircraftDefaults();

  cfg.heroEyebrow=fieldValue(settings,"hero_eyebrow",cfg.heroEyebrow);
  cfg.heroTitle=fieldValue(settings,"hero_title",cfg.heroTitle);
  cfg.heroIntro=fieldValue(settings,"hero_intro",cfg.heroIntro);

  cfg.stats=[
    {value:fieldValue(settings,"bubble_1_value",cfg.stats[0].value),text:fieldValue(settings,"bubble_1_text",cfg.stats[0].text)},
    {value:fieldValue(settings,"bubble_2_value",cfg.stats[1].value),text:fieldValue(settings,"bubble_2_text",cfg.stats[1].text)},
    {value:fieldValue(settings,"bubble_3_value",cfg.stats[2].value),text:fieldValue(settings,"bubble_3_text",cfg.stats[2].text)}
  ];

  cfg.introLabel=fieldValue(settings,"intro_label",cfg.introLabel);
  cfg.introTitle=fieldValue(settings,"intro_title",cfg.introTitle);
  cfg.introText=fieldValue(settings,"intro_text",cfg.introText);
  cfg.note=fieldValue(settings,"bottom_note",cfg.note);

  return cfg;
}

function renderActualAircraftPagePreview(){
  var cfg=aircraftPreviewConfigFromPageText();
  var stats=Array.isArray(cfg.stats)?cfg.stats:[];
  var rows=Array.isArray(cfg.aircraft)?cfg.aircraft.slice():[];
  ...
  function emptyAircraft(){
    return '<div class="aero-empty-message">Aircraft information is not available at this time.</div>';
  }
  ...
  return '<div class="aero-fleet-page"><div class="aero-fleet-shell">'+
    ...
    '<main class="aero-fleet-main">'+
      ...
      '</section>'+
      '<section class="aero-aircraft-list">'+(rows.length?rows.map(function(){return '';}).join(''):emptyAircraft())+'</section>'+
...
function renderSelectedPagePreview(){
  ...
  var bubble1Text=fieldValue(settings,"bubble_1_text","");
  var bubble2Value=fieldValue(settings,"bubble_2_value","MEDIA");
  var bubble2Text=fieldValue(settings,"bubble_2_text","");
  var bubble3Value=fieldValue(settings,"bubble_3_value","SAFE");
  var bubble3Text=fieldValue(settings,"bubble_3_text","");
...
function renderFrame(){
  ...
  mount.innerHTML='<div id="'+ROOT_ID+'" class="'+(state.loading?'ds-skeleton ':'')+(state.fullPreview?'ds-full-preview ':'')+(state.sideOpen?'ds-page-text-mode':'')+'">'+
