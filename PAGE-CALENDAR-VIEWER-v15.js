/* PAGE-CALENDAR-VIEWER-v15.js - BEGIN */
(function(){
"use strict";
...
function ensureComponents(){var base=componentBaseUrl();return COMPONENT_FILES.reduce(function(p,file){return p.then(function(){return loadScriptOnce(base + file + "?v=calendar-viewer-rsvp-button-v15");});},Promise.resolve());}
...
function rsvpUrl(ev){
  var id=eventId(ev);
  if(!id)return "";
  return "/event-rsvp?event_id="+encodeURIComponent(id);
}
function rsvpActionHtml(ev){
  if(!ev||!ev.rsvp_on)return "";
  var url=rsvpUrl(ev);
  if(!url)return "";
  return '<a class="cv-modal-link" href="'+esc(url)+'">RSVP / View Attendees</a>';
}
...
function renderModal(){
  ...
  root.innerHTML='<div class="cv-modal-backdrop" data-cv-modal-backdrop="1" role="presentation"><section class="cv-modal" role="dialog" aria-modal="true" aria-label="'+esc(ev.name||"Event details")+'"><header class="cv-modal-head"><div><span class="cv-pill">'+esc(ev.event_type_name||"Event")+'</span><h2 class="cv-modal-title">'+esc(ev.name||"Untitled Event")+'</h2></div><button type="button" class="cv-modal-close" data-cv-close-modal="1" aria-label="Close event details">×</button></header><div class="cv-modal-body"><div class="cv-modal-main">'+modalField("Date and Time",timeRange(ev))+modalField("Location",loc)+modalField("Address",addr)+(ev.short_note?'<div class="cv-modal-note">'+esc(ev.short_note)+'</div>':'')+(details?'<div class="cv-modal-note">'+esc(details)+'</div>':'')+'<div class="cv-modal-actions">'+rsvpActionHtml(ev)+urlHtml("Map",map,false)+urlHtml("Zoom",zoom,true)+urlHtml("Document",doc,true)+'</div></div><aside class="cv-modal-side">'+(ev.image_url?'<img class="cv-modal-img" src="'+esc(ev.image_url)+'" alt="'+esc(ev.name||"Event image")+'">':'')+modalField("Access",ev.access_label||ev.access_opt||ev.access||"")+modalField("RSVP",ev.rsvp_on?"RSVP is open for this event.":"Not enabled")+'</aside></div></section></div>';
}
...
/* PAGE-CALENDAR-VIEWER-v15.js - END */
