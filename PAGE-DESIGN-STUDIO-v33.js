/* PAGE-DESIGN-STUDIO-v33.js | Supabase style diagnostic mode for Design Studio preview | Generated: 2026-06-01 */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v33";
...
var STYLE_ID="syncetc-design-studio-v33-style";
...
function renderAuthPanel(){
  ...
  return '<div class="ds-auth-panel">'+
    ...
    styleDiagnosticHtml()+'<div><label>Password</label><input type="password" autocomplete="current-password" data-ds-auth-password></div>'+
...
function designPresetSource(){
  var d=obj(state.data);
  return obj(d.design_presets||d.presets||d.preset_lists||d);
}

function settingSource(){
  var d=obj(state.data);
  return obj(d.customer_site_settings||d.site_settings||d.settings||d.customer_settings||d);
}
...
function computeStyleDiagnostic(){
  var d=obj(state.data);
  var missing=[];
  if(!d || !Object.keys(d).length)missing.push("customer/style response");

  var presets=designPresetSource();
  if(!countList(presets.profiles))missing.push("design profiles");
  if(!countList(presets.layouts)&&!countList(presets.layout_presets))missing.push("layout presets");
  if(!countList(presets.styles)&&!countList(presets.style_presets))missing.push("style presets");
  if(!countList(presets.typography)&&!countList(presets.typography_presets))missing.push("typography presets");
  if(!countList(presets.colors)&&!countList(presets.color_presets))missing.push("color presets");
  if(!countList(presets.effects)&&!countList(presets.effect_presets))missing.push("effect presets");

  var settings=settingSource();
  var hasAssignment =
    clean(settings.active_design_profile_key||settings.design_profile_key||settings.active_profile_key) ||
    clean(settings.active_layout_key||settings.layout_key) ||
    clean(settings.active_style_key||settings.style_key) ||
    clean(settings.active_typography_key||settings.typography_key) ||
    clean(settings.active_color_key||settings.color_key) ||
    clean(settings.active_effect_key||settings.effect_key);

  if(!hasAssignment)missing.push("active customer style assignment");

  state.styleDiagnostic={missing:missing,ok:missing.length===0};
...
function styleDiagnosticHtml(){
  ...
  return '<div class="ds-supabase-style-warning">'+
    '<strong>SUPABASE STYLE VARIABLES MISSING</strong>'+
    '<span>Build/test diagnostic mode is active. This preview is using fallback styling because required Supabase-driven design settings were not found.</span>'+
    '<div class="ds-missing-list">'+esc(diag.missing.join(", "))+'</div>'+
  '</div>';
}
...
function bindFrame(){
  ...
  r.querySelectorAll('[data-ds-action]').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      if(action==='side-close'){if(hasSideUnsaved()&&!window.confirm('Discard unapplied/unsaved Page Text changes and close?'))return;state.sideOpen=false;state.sideDirty=false;state.sideAppliedDirty=false;state.sideDraftValues=null;state.sideAppliedValues=null;state.notice='Page Text closed.';renderFrame();return;}
