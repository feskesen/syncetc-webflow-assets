/* PAGE-DESIGN-STUDIO-v16.js | platform admin login for protected saves | Generated: 2026-06-01 09:55:00 UTC */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v16";
...
var STYLE_ID="syncetc-design-studio-v16-style";
  ...
  r.querySelectorAll('[data-ds-action]').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      if(action==='toggle-auth'){
        if(state.authEmail){clearAuthSession();state.notice='Signed out.';state.error='';state.authOpen=false;hydrateControls();updateStatus();return;}
        state.authOpen=!state.authOpen;hydrateControls();updateStatus();return;
      }
...
function saveNewDesignProfile(){
  ...
  var name=window.prompt('New Design Profile name:','');
  name=clean(name);
  if(!name){state.notice='New Design Profile cancelled.';state.error='';updateStatus();return;}

  var key=slugifyProfileName(name);
  if(!key){state.error='Design Profile name must contain at least one letter or number.';state.notice='';updateStatus();return;}

  state.saving=true;
  state.error='';
  state.notice='Saving new Design Profile...';
  hydrateControls();
  updateStatus();
  ...
  .then(function(body){
    var profile=(body&&body.result)||body.design_profile||null;
    ...
    state.dirty=true;
    state.error='';
    state.notice='New Design Profile saved: '+name+'. Use Save to customer to assign it.';
    hydrateControls();
    updateStatus();
    updatePreview(false);
  })
