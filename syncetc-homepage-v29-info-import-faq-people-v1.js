/* syncetc-homepage-v29-info-import-faq-people-v1.js - BEGIN */
(function () {
  "use strict";

  const VERSION = "JS v29-info-import-faq-people-v1";
  const FILE_NAME = "syncetc-homepage-v29-info-import-faq-people-v1.js";
  const MOUNT_ID = "syncetc-webflow-mount";
  const ROOT_ID = "syncetc-v29-root";
  const STYLE_ID = "syncetc-v29-style";
  ...
  function render() {
    ...
    mount.innerHTML = `<div id="${ROOT_ID}" class="syncetc-v28" style="--se-customer-bg:url('${cssUrl(base.backgroundUrl)}')"><div class="se-v28-version">${esc(VERSION)} loaded</div><div class="se-v28-topline"><span>SyncEtc versioned renderer · ${esc(FILE_NAME)}</span><span>${esc(state.customer)} · ${esc(currentPageLabel())}</span></div>${renderWorkbench(base)}${renderHeader(base)}${state.page === 'home' ? renderMarquee(base) : ""}${state.page === 'home' ? renderHome(base) : (state.page === 'info' ? renderInfoPage(base) : renderPageModules(base))}${renderFooter(base)}</div>`;
  }
...
/* syncetc-homepage-v29-info-import-faq-people-v1.js - END */
