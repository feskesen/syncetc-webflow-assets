...
(function () {
  ...
  function installStyles(){
    U().installStyle("COMPONENT-master-controls-v4-style", `
  ...
  function render(ctx){
    ...
    return `<section class="se-workbench" data-se-component="master-controls"><div class="se-site-editor-shell ${open?"is-open":""}">
      <div class="se-site-editor-toggle-row"><div class="se-site-editor-title"><h2>SyncEtc Site Editor</h2><span class="se-site-editor-pill">${U().esc(ctx.version||VERSION)}</span><span class="se-site-editor-pill">${U().esc(customer.shortName||customer.customerName||customerKey)}</span><span class="se-site-editor-sub">Calendar · View as ${U().esc(viewAs)}</span></div><button type="button" class="se-site-editor-toggle" data-se-toggle-editor>${open?"Close Site Editor":"Open Site Editor"}</button></div>
  ...
  function bind(ctx, root){
    ...
    target.addEventListener("click",function(e){
      ...
      var viewBtn=e.target.closest&&e.target.closest("[data-se-view-as]"); if(viewBtn){window.SyncEtc.Components.Utils.dispatch("syncetc:control-change",{field:"viewAs",value:viewBtn.getAttribute("data-se-view-as")||"public"});return;}
    ...
    target.addEventListener("change",function(e){var field=e.target&&e.target.getAttribute&&e.target.getAttribute("data-se-field");if(field)window.SyncEtc.Components.Utils.dispatch("syncetc:control-change",{field:field,value:e.target.value});var local=e.target&&e.target.getAttribute&&e.target.getAttribute("data-se-local");if(local)window.SyncEtc.Components.Utils.dispatch("syncetc:local-input",{field:local,value:e.target.value});});
    target.addEventListener("input",function(e){var local=e.target&&e.target.getAttribute&&e.target.getAttribute("data-se-local");if(local)window.SyncEtc.Components.Utils.dispatch("syncetc:local-input",{field:local,value:e.target.value});});
  }
  window.SyncEtc.Components.MasterControls={render:render,bind:bind,installStyles:installStyles,version:VERSION};
})();
/* COMPONENT-master-controls-v4.js - END */
