...
/* PAGE-DESIGN-STUDIO-v56.js | unsaved navigation guard for page settings | Generated: 2026-06-02 */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v56";
...
var STYLE_ID="syncetc-design-studio-v56-style";
...
/* v56 unsaved-change navigation guard - BEGIN */
function hasUnsavedPageSettingChanges(){
  return !!(state.sideDirty||state.sideAppliedDirty);
}

function unsavedPageSettingMessage(){
  return "You have unsaved Page Specific Settings changes. Discard them and continue?";
}

function confirmDiscardUnsavedPageSettingChanges(){
  if(!hasUnsavedPageSettingChanges())return true;
  return window.confirm(unsavedPageSettingMessage());
}

function resetSideEditStateAfterNavigation(){
  state.sideDirty=false;
  state.sideAppliedDirty=false;
...
function installBeforeUnloadGuard(){
  if(window.__SYNCETC_DS_BEFOREUNLOAD_V56)return;
  window.__SYNCETC_DS_BEFOREUNLOAD_V56=true;
...
/* v56 unsaved-change navigation guard - END */

/* v55 side drawer button wiring - BEGIN */

/* v56 selector safe-value tracking - BEGIN */
function markSafeNavigationSelectValues(){
  var r=root();
  if(!r)return;
  var selectors=[
    '[data-ds-customer-select]',
    '[data-ds-customer-key]',
    'select[name="customer"]',
    'select[name="customer_key"]',
  ...
  selectors.forEach(function(sel){
    r.querySelectorAll(sel).forEach(function(t){
      if(t.dataset&&t.dataset.lastSafeValue===undefined)t.dataset.lastSafeValue=t.value;
      if(t.getAttribute("data-last-safe-value")===null)t.setAttribute("data-last-safe-value",t.value);
    });
  });
}
/* v56 selector safe-value tracking - END */
...
function bindFrame(){
  ...
  r.addEventListener("change",function(e){
    var t=e.target;
    if(!t)return;
    var isCustomerSwitch=t.matches&&(
      t.matches('[data-ds-customer-select]')||
      t.matches('[data-ds-customer-key]')||
      t.matches('select[name="customer"]')||
      t.matches('select[name="customer_key"]')||
      t.matches('select[name="customerKey"]')
    );
    var isPageSwitch=t.matches&&(
      t.matches('[data-ds-page-select]')||
      t.matches('[data-ds-page-key]')||
      t.matches('select[name="page"]')||
      t.matches('select[name="page_key"]')||
      t.matches('select[name="pageKey"]')
    );
    if(!(isCustomerSwitch||isPageSwitch))return;
    if(!confirmDiscardUnsavedPageSettingChanges()){
      e.preventDefault();
      if(t.dataset&&t.dataset.lastSafeValue!==undefined)t.value=t.dataset.lastSafeValue;
      else if(t.getAttribute("data-last-safe-value")!==null)t.value=t.getAttribute("data-last-safe-value");
      return false;
    }
    resetSideEditStateAfterNavigation();
    if(t.dataset)t.dataset.lastSafeValue=t.value;
    t.setAttribute("data-last-safe-value",t.value);
  },true);
...
function resetPreview(){
  ...
  hydrateControls();
  markSafeNavigationSelectValues();
  updateStatus();
  updatePreview(false);
}
