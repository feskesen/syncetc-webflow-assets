...
/* PAGE-DESIGN-STUDIO-v74.js | hardened close guarded exit | Generated: 2026-06-02 */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v74";
...
var STYLE_ID="syncetc-design-studio-v74-style";
...
/* v74 hardened close guarded exit - BEGIN */
function v74HasUnsavedPageSpecificWork(){
  return !!(state.sideDirty||state.sideAppliedDirty);
}

function v74CloseSideDrawer(){
  if(v74HasUnsavedPageSpecificWork()){
    var ok=window.confirm("You have unsaved Page Specific Settings changes. Close and lose those changes, or cancel to keep editing?");
    if(!ok)return;
  }
...
if(!window.__SYNCETC_DS_BEFOREUNLOAD_V74){
  window.__SYNCETC_DS_BEFOREUNLOAD_V74=true;
  window.addEventListener("beforeunload",function(e){
    if(!v74HasUnsavedPageSpecificWork())return;
    e.preventDefault();
    e.returnValue="";
    return "";
  });
}
/* v74 hardened close guarded exit - END */
...
/* v74 removed legacy discard function */
...
function bindFrame(){
  ...
  r.querySelectorAll('[data-ds-action]').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      if(action==='side-close'){v74CloseSideDrawer();return;}
      if(action==='apply-page-text'){applyPageText();return;}
      if(action==='save-page-text'){savePageText();return;}
      if(action==='discard-page-text'){v74CloseSideDrawer();return;}
...
/* v74 removed legacy discard capture fallback */
...
/* v74: boot call removed; local beforeunload guard installed below */

/* v74 close capture fallback - BEGIN */
if(!window.__SYNCETC_DS_CLOSE_CAPTURE_V74){
  window.__SYNCETC_DS_CLOSE_CAPTURE_V74=true;
  document.addEventListener("click",function(e){
    var btn=e.target&&e.target.closest?e.target.closest('[data-ds-action="side-close"]'):null;
    if(!btn)return;
    e.preventDefault();
    e.stopImmediatePropagation();
    v74CloseSideDrawer();
...
/* v74 close capture fallback - END */


/* v74 close diagnostic - BEGIN */
window.SYNCETC_V74_CLOSE_STATE=function(){
  var out={
    version:VERSION,
    sideOpen:state.sideOpen,
    sideDirty:state.sideDirty,
    sideAppliedDirty:state.sideAppliedDirty,
    hasUnsaved:v74HasUnsavedPageSpecificWork(),
    closeCaptureInstalled:!!window.__SYNCETC_DS_CLOSE_CAPTURE_V74,
    beforeUnloadInstalled:!!window.__SYNCETC_DS_BEFOREUNLOAD_V74
  };
  console.log("SYNCETC_V74_CLOSE_STATE",out);
  return out;
};
/* v74 close diagnostic - END */

/* PAGE-DESIGN-STUDIO-v74.js - END */
