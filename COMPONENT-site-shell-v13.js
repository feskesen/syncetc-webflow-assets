...
(function () {
  ...
  function installDrawerStyles(){
    window.SyncEtc.Components.Utils.installStyle("COMPONENT-site-shell-v13-drawer-style",`
  ...
  function create(mountId,options){
    ...
    if(!authListenerInstalled){
      ...
      function activateAdminLayer(){
        ...
        refreshSecurityAndRender();
      }
      try{document.addEventListener("syncetc:auth-soft-change",function(){if(state.adminReady)refreshSecurityAndRender();});}catch(e){}
      setTimeout(activateAdminLayer,5000);
    }
    var api={render:render,customer:customer,loadCustomer:loadCustomer,getState:getState,updateState:updateState,setLocal:setLocal,setViewAs:setViewAs,setCustomerKey:setCustomerKey,setEditorOpen:setEditorOpen,setCustomerSettingsOpen:setCustomerSettingsOpen,setDrawerOpen:setDrawerOpen,setDrawerTab:setDrawerTab,getSavePayload:getSavePayload,refreshSecurityAndRender:refreshSecurityAndRender,setAdminReady:function(value){state.adminReady=!!value;render(state.lastPageHtml||"");}};
    return api;
  }
  window.SyncEtc.Components.SiteShell={create:create,version:VERSION};
})();
/* COMPONENT-site-shell-v13.js - END */
