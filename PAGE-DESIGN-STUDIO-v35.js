/* PAGE-DESIGN-STUDIO-v35.js | detailed Supabase style inheritance audit | Generated: 2026-06-01 */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v35";
...
var STYLE_ID="syncetc-design-studio-v35-style";
...
function listHasKey(list,key){
  ...
  if(Array.isArray(list)){
    return list.some(function(item){
      item=obj(item);
      return clean(item.key||item.slug||item.name||item.id)===key;
    });
  }
  if(typeof list==="object"){
    if(Object.prototype.hasOwnProperty.call(list,key))return true;
    return Object.keys(list).some(function(k){
      var item=obj(list[k]);
      return k===key || clean(item.key||item.slug||item.name||item.id)===key;
    });
  }
  return false;
}
...
function getPresetBucket(names){
  var d=dataRoot();
  var sources=[
    d.design_presets,
    d.presets,
  ...
  for(var i=0;i<sources.length;i++){
    for(var j=0;j<names.length;j++){
      var found=firstList(sources[i][names[j]]);
...
function selectedValue(setting,names){
  for(var i=0;i<names.length;i++){
    var v=clean(setting[names[i]]);
    if(v)return v;
  }
  return "";
}
...
function auditOne(label,settingNames,presetNames,required){
  var setting=getSettingObject();
  var selected=selectedValue(setting,settingNames);
  var bucket=getPresetBucket(presetNames);
  var hasBucket=!!bucket;
  var resolves=selected ? listHasKey(bucket,selected) : false;

  var status="ok";
  var detail="";
  if(!Object.keys(dataRoot()).length){
    status="fail";
    detail="No Supabase customer/style response available.";
  }else if(required && !selected){
    status="fail";
    detail="Returned but empty. Expected one of: "+settingNames.join(", ");
  }else if(selected && !hasBucket){
    status="warn";
    detail="Selected key '"+selected+"' exists, but preset list was not returned. Checked: "+presetNames.join(", ");
  }else if(selected && hasBucket && !resolves){
    status="fail";
    detail="Selected key '"+selected+"' was not found in returned preset list. Checked: "+presetNames.join(", ");
  }else if(selected && resolves){
    detail="Applied key '"+selected+"' resolved in preset list.";
  }else if(!required && !selected){
    status="warn";
    detail="Optional selection is empty. Checked: "+settingNames.join(", ");
  }else{
    detail="Available.";
  }

  return {label:label,status:status,detail:detail,selected:selected,hasBucket:hasBucket};
}

function computeStyleDiagnostic(){
  var audits=[
    auditOne("Customer profile",["customer_key","customer_slug","customer_id"],["customers","customer_profiles","profiles"],false),
...
function styleDiagnosticHtml(){
  ...
  return '<div class="ds-supabase-style-warning">'+
    ...
    '<span>Build/test diagnostic mode is active. The items below did not fully inherit from the Supabase design/profile data.</span>'+
