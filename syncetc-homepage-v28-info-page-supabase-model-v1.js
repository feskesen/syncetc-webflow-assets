/* syncetc-homepage-v28-info-page-supabase-model-v1.js - BEGIN */
(function () {
  "use strict";

  const VERSION = "JS v28-info-page-v1";
  const FILE_NAME = "syncetc-homepage-v28-info-page-supabase-model-v1.js";
  const MOUNT_ID = "syncetc-webflow-mount";
  const ROOT_ID = "syncetc-v28-root";
  const STYLE_ID = "syncetc-v28-style";
  ...
  const INFO_PAGE_DEFAULTS = {
    "150th Aero Flying Club": {
      ...
      officers: [
        { title:"President", name:"Frank Eskesen" },
        { title:"Vice President", name:"Frank Generoso" },
        { title:"Treasurer", name:"Bryan Barhydt" },
        { title:"Maintenance Officer", name:"Flavio Coste" },
        { title:"Assistant Maintenance Officer", name:"Karl Beamrath" },
        { title:"Safety Officer", name:"Jonathan Friedman" },
        { title:"Secretary", name:"Tomasz Surowiecki" }
      ],
      faqs: [
        { q:"What qualifications must I have?", a:"Applicants should hold at least a private pilot certificate and satisfy current Club checkout, currency, and membership requirements." },
        { q:"What are the initial costs to join the Club?", a:"Costs may include initiation-related amounts, refundable bond/share amounts, dues, and any current assessments or deposits. Current amounts should be confirmed through the Board." },
        { q:"What are the monthly dues?", a:"Monthly dues may change over time and should be confirmed through current Club materials or the Board." },
        { q:"What is the hourly rate for flight time?", a:"Current aircraft hourly rates and fuel-related charges should be confirmed through current Club materials, Flight Circle, or the Board." },
        { q:"Are there additional costs beyond dues and flight time?", a:"Additional costs may include assessments, checkouts, training, supplies, or other Club-approved charges depending on current Club rules and operating needs." },
        { q:"What other responsibilities do members have?", a:"Members are expected to follow Club rules, maintain currency, participate responsibly, preserve aircraft condition, and communicate safety or maintenance concerns promptly." },
        { q:"How do I schedule aircraft time?", a:"Aircraft scheduling is handled through the Club's scheduling system. Members receive access after applicable onboarding and checkout requirements are met." },
        { q:"Wow, this sounds amazing! How do I apply?", a:"Use the Apply page or contact the Board to begin the membership process." }
      ],
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .syncetc-v28 { min-height:100vh; font-family:Arial, Helvetica, sans-serif; color:var(--aero-text); background:linear-gradient(rgba(240,247,252,.78),rgba(240,247,252,.80)), var(--se-customer-bg); background-position:center; background-size:cover; background-attachment:fixed; padding:10px 0 48px; box-sizing:border-box; }
      .syncetc-v28 * { box-sizing:border-box; }
      .se-v28-version { position:fixed; right:12px; top:12px; z-index:999999; padding:8px 11px; border-radius:999px; background:#12365a; color:#fff; font-size:12px; font-weight:900; box-shadow:0 8px 24px rgba(0,0,0,.25); pointer-events:none; }
      .se-v28-topline { max-width:1180px; margin:0 auto 8px; padding:0 18px; display:flex; justify-content:space-between; align-items:center; gap:10px; color:#12365a; font-size:11px; font-weight:900; }
      .se-v28-topline span { background:rgba(255,255,255,.72); border:1px solid var(--aero-border); border-radius:999px; padding:6px 9px; }
      ...
      .aero-faq-answer { display:none; padding:0 15px 16px; color:var(--aero-text); font-size:14px; line-height:1.65; }
      .aero-faq-item.is-open .aero-faq-answer { display:block; }
  ...
  function navRows() {
    ...
    return `
      <nav class="club-nav-row public"><div class="club-nav-inner"><div class="club-nav-label">Public</div><div class="club-nav-links">
        ${link("home","Home")}${link("info","Info")}${navLink("Aircraft")}${link("events","Calendar")}${link("gallery","Gallery")}${navLink("Scheduler")}${navLink("Apply")}${navLink("Contact")}
      </div></div></nav>
  ...
  function render() {
    ...
    mount.innerHTML = `<div id="${ROOT_ID}" class="syncetc-v28" style="--se-customer-bg:url('${cssUrl(base.backgroundUrl)}')"><div class="se-v28-version">JS v27 loaded</div><div class="se-v28-topline"><span>SyncEtc versioned renderer · ${esc(FILE_NAME)}</span><span>${esc(state.customer)} · ${esc(currentPageLabel())}</span></div>${renderWorkbench(base)}${renderHeader(base)}${renderMarquee(base)}${state.page === 'home' ? renderHome(base) : (state.page === 'info' ? renderInfoPage(base) : renderPageModules(base))}${renderFooter(base)}</div>`;
  }
  ...
  function exportJson() {
    ...
    a.href = url;
    a.download = "syncetc-v28-display-settings-" + Date.now() + ".json";
...
/* syncetc-homepage-v28-info-page-supabase-model-v1.js - END */
