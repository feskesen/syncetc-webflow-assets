/* PAGE-CALENDAR-VIEWER-v5.js - BEGIN */
(function(){
"use strict";
...
function ensureComponents(){var base=componentBaseUrl();return COMPONENT_FILES.reduce(function(p,file){return p.then(function(){return loadScriptOnce(base + file + "?v=calendar-viewer-startup-lock-v5");});},Promise.resolve());}
var VERSION="PAGE-CALENDAR-VIEWER-v5",EDGE_READ_URL="https://ocdaohkiwonjmirqkjww.supabase.co/functions/v1/syncetc-calendar-viewer-read";
var state={shell:null,events:[],visibleEvents:[],selectedEvent:null,dateFilter:"upcoming",typeFilter:"all",search:"",loading:true,error:"",lastCustomerKey:"",refreshTimer:null,refreshBusy:false,refreshQueued:false,startupComplete:false,startupIgnoreUntil:Date.now()+2500};
...
function fetchData(){state.loading=true;state.error="";return fetch(EDGE_READ_URL+"?customer_key="+encodeURIComponent(activeCustomerKey()),{credentials:"omit"}).then(function(r){return r.json().catch(function(){return null;}).then(function(body){if(!r.ok||!body||body.ok===false)throw new Error((body&&body.error)||"Calendar read failed");return body;});}).then(function(body){state.events=(body.events||[]).map(normalizeEvent).sort(sortEvents);state.loading=false;}).catch(function(err){state.loading=false;state.error=err&&err.message?err.message:"Calendar could not load.";});}
...
function renderBody(){installStyles();return '<div class="calendar-viewer-page"><div class="cv-shell"><section class="cv-hero"><div><div class="cv-eyebrow">'+esc(activeCustomerLabel())+'</div><h1 data-cv-live-title>'+esc(pageValue("calendar.title","Calendar"))+'</h1><p data-cv-live-intro>'+esc(pageValue("calendar.intro","Upcoming meetings, fly-outs, work sessions, and other events."))+'</p><p>'+esc(pageValue("calendar.helperText","Select an event card for details, links, notes, and location information."))+'</p></div></section><section class="cv-toolbar"><div><label>Search Events</label><input id="cv-search" class="cv-input" type="search" placeholder="Search title, type, location, address, or notes" value="'+esc(state.search)+'"></div><div id="cv-filters" class="cv-filters"></div></section><section id="cv-status" class="cv-meta">Loading events...</section><main id="cv-grid" class="cv-grid"></main></div><div id="cv-modal-root"></div></div>';}
function createFilters(){var box=byId("cv-filters");if(!box)return;var types={};state.events.forEach(function(ev){if(ev.event_type_name)types[ev.event_type_name]=true;});box.innerHTML='<button type="button" class="cv-filter '+(state.dateFilter==="upcoming"?"active":"")+'" data-cv-date="upcoming">Upcoming</button><button type="button" class="cv-filter '+(state.dateFilter==="past"?"active":"")+'" data-cv-date="past">Past</button><button type="button" class="cv-filter '+(state.dateFilter==="all"?"active":"")+'" data-cv-date="all">All Dates</button><button type="button" class="cv-filter '+(state.typeFilter==="all"?"active":"")+'" data-cv-type="all">All Types</button>'+Object.keys(types).sort().map(function(t){return'<button type="button" class="cv-filter '+(state.typeFilter===t?"active":"")+'" data-cv-type="'+esc(t)+'">'+esc(t)+'</button>';}).join("");}
...
function renderEvents(){var status=byId("cv-status");if(status){status.textContent=state.error?"Calendar unavailable.":state.loading?"Loading events...":state.visibleEvents.length+" visible event"+(state.visibleEvents.length===1?"":"s")+".";}var grid=byId("cv-grid");if(!grid)return;if(state.error){grid.innerHTML='<div class="cv-error">'+esc(state.error)+'</div>';return;}if(state.loading){grid.innerHTML='<div class="cv-empty">Loading events...</div>';return;}grid.innerHTML=state.visibleEvents.length?state.visibleEvents.map(card).join(""):'<div class="cv-empty">No events match the current filters.</div>';}
...
function renderCalendarOnly(){installStyles();createFilters();applyFilters();var search=byId("cv-search");if(search)search.value=state.search;}
function scheduleRefresh(reason){
  if(!state.startupComplete && Date.now()<state.startupIgnoreUntil){
    state.refreshQueued=true;
    return;
  }
  if(state.refreshTimer)clearTimeout(state.refreshTimer);
  state.refreshTimer=setTimeout(function(){runStableRefresh(reason||"scheduled");},300);
}
function runStableRefresh(reason){
  if(state.refreshBusy){state.refreshQueued=true;return;}
  if(!state.startupComplete && Date.now()<state.startupIgnoreUntil){state.refreshQueued=true;return;}
  state.refreshBusy=true;
  var beforeKey=activeCustomerKey();
  Promise.resolve()
    .then(function(){
      if(window.SyncEtc&&window.SyncEtc.SecurityContext&&window.SyncEtc.SecurityContext.refresh){
        return window.SyncEtc.SecurityContext.refresh(beforeKey);
      }
    })
    .then(function(){
      ...
      state.lastCustomerKey=afterKey||state.lastCustomerKey;
