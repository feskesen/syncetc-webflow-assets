/* COMPONENT-customer-settings-v4.js - BEGIN */
(function(){
"use strict";
window.SyncEtc=window.SyncEtc||{};
window.SyncEtc.Components=window.SyncEtc.Components||{};
var VERSION="COMPONENT-customer-settings-v4";
...
function installStyles(){
  U().installStyle("COMPONENT-customer-settings-v4-style",`
    .se-customer-settings{max-width:1180px;margin:8px auto 16px;padding:0 18px}
    .se-customer-settings-shell{background:rgba(255,255,255,.96);border:1px solid var(--se-aero-border);border-radius:20px;box-shadow:0 10px 26px rgba(12,38,64,.10);overflow:hidden}
    .se-customer-settings-top{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:10px 14px;background:rgba(234,245,255,.92);border-bottom:1px solid var(--se-aero-border);color:var(--se-aero-navy)}
    .se-customer-settings-title{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
    .se-customer-settings-title h2{margin:0;font-size:15px;color:var(--se-aero-navy)}
    .se-customer-settings-pill{display:inline-flex;padding:4px 9px;border-radius:999px;background:#fff;border:1px solid var(--se-aero-border);font-size:10.5px;font-weight:900;text-transform:uppercase;color:var(--se-aero-navy)}
    .se-customer-settings-toggle{border:1px solid rgba(18,54,90,.20);border-radius:999px;padding:8px 12px;background:#fff;color:var(--se-aero-navy);font-size:12px;font-weight:900;cursor:pointer}.se-cs-top-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end}.se-cs-top-identity{display:flex;align-items:center;gap:6px;flex-wrap:wrap;justify-content:flex-end;font-size:10.5px;font-weight:900;color:var(--se-aero-navy)}.se-cs-top-identity span{display:inline-flex;border:1px solid var(--se-aero-border);background:#fff;border-radius:999px;padding:4px 8px}.se-cs-top-signout{border:1px solid rgba(18,54,90,.20);border-radius:999px;padding:7px 10px;background:#fff;color:var(--se-aero-navy);font-size:11px;font-weight:900;cursor:pointer}
    .se-customer-settings-body{display:none;padding:16px;background:#fff}
    .se-customer-settings-shell.is-open .se-customer-settings-body{display:block}
    .se-customer-settings-note{margin:0 0 12px;color:#657789;font-size:12px;line-height:1.4;font-weight:700}.se-cs-identity{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-bottom:12px;padding:10px;border:1px solid var(--se-aero-border);border-radius:14px;background:rgba(234,245,255,.66)}.se-cs-id-item{font-size:11px;line-height:1.25;color:#4f6073}.se-cs-id-item strong{display:block;color:var(--se-aero-navy);font-size:11px;text-transform:uppercase;letter-spacing:.04em}.se-cs-signout{justify-self:end;align-self:center;border:1px solid rgba(18,54,90,.20);border-radius:999px;padding:8px 11px;background:#fff;color:var(--se-aero-navy);font-size:11px;font-weight:900;cursor:pointer}
    .se-customer-settings-row{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px}
    .se-customer-settings-control{display:block}
    .se-customer-settings-control span{display:block;margin-bottom:5px;color:var(--se-aero-navy);font-size:11px;font-weight:900;text-transform:uppercase}
    .se-customer-settings-control input,.se-customer-settings-control textarea{width:100%;border:1px solid rgba(18,54,90,.20);border-radius:11px;padding:9px 10px;font:inherit;font-size:13px;background:#fff;color:var(--se-aero-text)}
    .se-customer-settings-control textarea{min-height:70px;resize:vertical}
    .se-customer-settings-control small{display:block;margin-top:4px;color:#657789;font-size:11px}
    .se-customer-settings-actions{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}
    .se-customer-settings-actions button{border:1px solid rgba(18,54,90,.20);border-radius:999px;padding:9px 12px;background:var(--se-aero-navy);color:#fff;font-weight:900;cursor:pointer}
    .se-customer-settings-actions button.secondary{background:#fff;color:var(--se-aero-navy)}
    ...
    .se-customer-settings-status.warn{color:#8f2424}
    .se-customer-settings-status.good{color:#247245}
    @media(max-width:900px){.se-customer-settings-row{grid-template-columns:1fr}.se-cs-identity{grid-template-columns:1fr}.se-cs-signout{justify-self:start}.se-customer-settings-top{align-items:flex-start;flex-direction:column}.se-cs-top-actions{justify-content:flex-start}}
  `);
}
...
function render(ctx){
  ...
  return `<section class="se-customer-settings"><div class="se-customer-settings-shell ${open?"is-open":""}" data-se-customer-settings>
    <div class="se-customer-settings-top">
      <div class="se-customer-settings-title"><h2>${U().esc(customerName)} · Calendar Page Settings</h2><span class="se-customer-settings-pill">page-scoped</span><span class="se-customer-settings-pill">Customer Settings</span></div>
      <div class="se-cs-top-actions"><div class="se-cs-top-identity"><span>${U().esc(actor)}</span><span>${U().esc(customerRole)}</span><span>${U().esc(customerName)}</span></div><button type="button" class="se-cs-top-signout" data-cs-signout>Not you? Sign out</button><button type="button" class="se-customer-settings-toggle" data-cs-toggle>${open?"Close Customer Settings":"Open Customer Settings"}</button></div>
    </div>
    <div class="se-customer-settings-body">
      <div class="se-cs-identity"><div class="se-cs-id-item"><strong>Signed in as</strong>${U().esc(actor)}</div><div class="se-cs-id-item"><strong>Customer</strong>${U().esc(customerName)}</div><div class="se-cs-id-item"><strong>Customer role</strong>${U().esc(customerRole)}</div><div class="se-cs-id-item"><strong>Platform role</strong>${U().esc(platformRole||"n/a")}</div><button type="button" class="se-cs-signout" data-cs-signout>Not you? Sign out</button></div>
...
/* COMPONENT-customer-settings-v4.js - END */
