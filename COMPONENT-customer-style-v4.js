...
(function () {
  ...
  function buildConfig(customerKeyOrName, settingsBundle, overrides){
    ...
    var chosenPresetKey = overrides.stylePresetKey || overrides.style_preset_key || settings.style_preset_key || "classic-aviation";
    var preset = presetByKey(presets, chosenPresetKey);
    ...
    config.content = deepMerge(settings.content_overrides || {}, overrides.content || {});
    config.modules = settings.enabled_modules || {};
    config.rawSiteSettings = settings;
    config.availablePresets = presets;
    var finalConfig = deepMerge(config, overrides || {});
    finalConfig.theme = theme;
    finalConfig.availablePresets = presets;
    return finalConfig;
  }
...
/* COMPONENT-customer-style-v4.js - END */
