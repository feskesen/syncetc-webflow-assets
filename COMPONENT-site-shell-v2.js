...
(function () {
  ...
  var VERSION="COMPONENT-site-shell-v2";
  ...
  function installDrawerStyles(){
    window.SyncEtc.Components.Utils.installStyle("COMPONENT-site-shell-v2-drawer-style",`
  ...
  function create(mountId,options){
    ...
    function canSite(){
      try{
        ...
        var s=window.SyncEtc.SecurityContext&&window.SyncEtc.SecurityContext.getSnapshot?window.SyncEtc.SecurityContext.getSnapshot():{};
        return !!(s.is_syncetc_platform_admin||s.is_syncetc_super_admin||s.platform_role==="super_admin"||s.platform_role==="admin");
    ...
    function canCustomer(){
      try{
        ...
        var s=window.SyncEtc.SecurityContext&&window.SyncEtc.SecurityContext.getSnapshot?window.SyncEtc.SecurityContext.getSnapshot():{};
        return !!(s.active_customer_is_admin||s.active_customer_role==="owner"||s.active_customer_role==="admin"||s.active_customer_role==="manager");
      }catch(e){return false;}
    }
    ...
    function drawerHtml(c,audience){
      ...
      return `<button type="button" class="syncetc-admin-fab" data-se-drawer-open><span>Edit</span><small>${site?"platform v2":"customer v2"}</small></button>
    ...
    function refreshSecurityAndRender(){
      try{
        if(window.SyncEtc.SecurityContext&&window.SyncEtc.SecurityContext.refresh){
          window.SyncEtc.SecurityContext.refresh(state.customerKey||"demo_flying_club").then(function(){render(state.lastPageHtml||"");});
        }
      }catch(e){}
    }
    try{document.addEventListener("syncetc:auth-soft-change",refreshSecurityAndRender);}catch(e){}
    var api={render:render,customer:customer,loadCustomer:loadCustomer,getState:getState,setLocal:setLocal,setViewAs:setViewAs,setCustomerKey:setCustomerKey,setEditorOpen:setEditorOpen,setCustomerSettingsOpen:setCustomerSettingsOpen,setDrawerOpen:setDrawerOpen,setDrawerTab:setDrawerTab,getSavePayload:getSavePayload,refreshSecurityAndRender:refreshSecurityAndRender};
    return api;
  }
