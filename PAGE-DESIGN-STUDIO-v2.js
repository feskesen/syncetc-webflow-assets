/* PAGE-DESIGN-STUDIO-v2.js | sticky layered design studio | Generated: 2026-06-01 02:45:00 UTC */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v2";
...
var STYLE_ID="syncetc-design-studio-v2-style";
var CONTROL_ORDER=["customer","layout","style","typography","color","effect"];
...
function cloneSelected(v){return {layout:v.layout,style:v.style,typography:v.typography,color:v.color,effect:v.effect};}
function sameSelected(a,b){return clean(a.layout)===clean(b.layout)&&clean(a.style)===clean(b.style)&&clean(a.typography)===clean(b.typography)&&clean(a.color)===clean(b.color)&&clean(a.effect)===clean(b.effect);}
...
function installStyles(){
  ...
  style.textContent='\
    ...
    #'+ROOT_ID+'{--ds-shell-max:1240px;--ds-sticky-top:0px;--ds-control-h:126px;--ds-primary:#12365a;--ds-accent:#2f80c4;--ds-soft:#eaf5ff;--ds-text:#17212b;--ds-muted:#5d6b78;--ds-bg:#fff;--ds-font:Arial,Helvetica,sans-serif;--ds-radius:22px;--ds-button-radius:999px;--ds-eyebrow-radius:999px;--ds-gap:18px;--ds-shadow:0 10px 28px rgba(12,38,64,.10);--ds-border:1px solid rgba(18,54,90,.14);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;color:#17212b;min-height:100vh;background:linear-gradient(180deg,#f7fbff 0%,#ffffff 220px);}\
    ...
    #'+ROOT_ID+' .ds-preview-panel{border:1px solid rgba(18,54,90,.14);border-radius:28px;background:#fff;box-shadow:0 18px 48px rgba(12,38,64,.12);overflow:hidden;}\
    #'+ROOT_ID+' .ds-preview-head{display:flex;justify-content:space-between;gap:14px;align-items:center;padding:16px 18px;border-bottom:1px solid rgba(18,54,90,.10);background:#fbfdff;}\
    ...
    #'+ROOT_ID+' .ds-preview-frame{padding:18px;background:#f7fbff;min-height:660px;}\
    ...
    #'+ROOT_ID+' .ds-preview-header{display:grid;grid-template-columns:92px minmax(0,1fr) auto;gap:16px;align-items:center;margin-bottom:var(--ds-gap,22px);padding:var(--ds-header-pad,14px);border:var(--ds-border);border-radius:var(--ds-radius);box-shadow:var(--ds-shadow);background:var(--ds-card,#fff);}\
    #'+ROOT_ID+' .ds-logo-box{height:72px;border-radius:var(--ds-radius);border:var(--ds-border);background:var(--ds-soft);display:flex;align-items:center;justify-content:center;color:var(--ds-primary);font-weight:1000;}\
    ...
    #'+ROOT_ID+' .ds-card{padding:var(--ds-card-pad,20px);border:var(--ds-border);border-radius:var(--ds-radius);box-shadow:var(--ds-shadow);background:var(--ds-card,#fff);}\
    ...
    #'+ROOT_ID+' .ds-card-meta{margin-top:14px;padding-top:12px;border-top:1px solid rgba(18,54,90,.10);color:var(--ds-muted);font-size:12px;font-weight:800;}\
    ...
    @media(max-width:1080px){#'+ROOT_ID+' .ds-toolbar-grid{grid-template-columns:1fr 1fr 1fr;}#'+ROOT_ID+'{--ds-control-h:214px;}}\
    @media(max-width:760px){#'+ROOT_ID+' .ds-shell{padding:18px 14px 48px;}#'+ROOT_ID+' .ds-hero-top{display:block;}#'+ROOT_ID+' .ds-version{margin-top:12px;}#'+ROOT_ID+' .ds-toolbar-grid{grid-template-columns:1fr;}#'+ROOT_ID+'{--ds-control-h:430px;}#'+ROOT_ID+' .ds-toolbar-actions{display:block;}#'+ROOT_ID+' .ds-actions{justify-content:flex-start;margin-top:10px;}#'+ROOT_ID+' .ds-preview-head{display:block;}#'+ROOT_ID+' .ds-chip-row{justify-content:flex-start;margin-top:10px;}#'+ROOT_ID+' .ds-preview-header{grid-template-columns:1fr;}#'+ROOT_ID+' .ds-grid{grid-template-columns:1fr!important;}}\
  ';
...
function optionHtml(list,keyName,labelName,selected){
  return arr(list).map(function(x){
    ...
    var label=clean(x[labelName])||key;
...
function describe(list,keyName,key,descName,jsonName){
  ...
  return clean(p[descName])||JSON.stringify(obj(p[jsonName])).slice(0,140);
}
...
function cssVars(){
  ...
  out["--ds-radius"]=S.radiusScale==="none"?"0px":(S.radiusScale==="medium"?"12px":"22px");
  ...
  out["--ds-shadow"]=E.shadows==="none"?"none":(E.shadows==="minimal"?"0 3px 10px rgba(12,38,64,.06)":"0 10px 28px rgba(12,38,64,.10)");
  out["--ds-border"]=E.borders==="none"?"0":(E.borders==="hairline"?"1px solid rgba(18,54,90,.10)":"1px solid rgba(18,54,90,.14)");
  out["--ds-hero-bg"]=E.gradients==="none"?(C.background||"#ffffff"):"linear-gradient(135deg,"+(C.primary||"#12365a")+","+(C.accent||"#2f80c4")+")";
  out["--ds-hero-text"]=E.gradients==="none"?(C.text||"#17212b"):"#ffffff";
  out["--ds-hero-muted"]=E.gradients==="none"?(C.muted||"#5d6b78"):"rgba(255,255,255,.88)";
  out["--preview-radius"]=L.shell==="unboxed"?"0px":"24px";
  out["--preview-border"]=L.shell==="unboxed"?"0":"1px solid rgba(18,54,90,.14)";
  out["--preview-shadow"]=L.shell==="unboxed"?"none":"0 12px 34px rgba(12,38,64,.10)";
  ...
  out["--ds-card"]=L.sections==="floating"?"transparent":"#ffffff";
  out["--ds-card-pad"]=L.sections==="floating"?"22px 0":"20px";
    ...
    '<div class="ds-shell">'+
      ...
      '<div class="ds-toolbar-wrap"><section class="ds-toolbar" aria-label="Design Studio Controls">'+
        '<div class="ds-toolbar-grid">'+
          ...
          controlHtml("effect","Effects","effect")+
        '</div>'+
        '<div class="ds-toolbar-actions"><div class="ds-status" data-ds-status></div><div class="ds-actions"><button class="ds-btn" type="button" data-ds-action="save">Save to customer</button><button class="ds-btn secondary" type="button" data-ds-action="reset">Reset preview</button><button class="ds-btn secondary" type="button" data-ds-action="reload">Reload</button></div></div>'+
      '</section></div>'+
      '<section class="ds-preview-panel"><div class="ds-preview-head"><div><h2>Live preview</h2><p>Local preview updates before save. The control bar keeps fixed order and reserved height to avoid first-load hopping.</p></div><div class="ds-chip-row" data-ds-chips></div></div><div class="ds-preview-frame" data-ds-preview-frame></div></section>'+
    '</div>'+
  '</div>';
  state.booted=true;
  bindFrame();
...
function hydrateControls(){
  ...
  var lists=getPresetLists();
  var map={
    customer:{html:customerOptionsHtml(),value:state.customer_key,help:"Default preview profile is Demo Flying Club."},
  ...
  CONTROL_ORDER.forEach(function(key){
    ...
    if(sel){sel.innerHTML=map[key].html||'<option value="">No options</option>'; sel.value=map[key].value; sel.disabled=state.loading||state.saving;}
...
function updatePreview(immediate){
  ...
  var run=function(){
    var vars=cssVars();
    var chipHtml='<span class="ds-chip">'+esc(state.selected.layout)+'</span><span class="ds-chip">'+esc(state.selected.style)+'</span><span class="ds-chip">'+esc(state.selected.typography)+'</span><span class="ds-chip">'+esc(state.selected.color)+'</span><span class="ds-chip">'+esc(state.selected.effect)+'</span>';
    setHTML('[data-ds-chips]',chipHtml);
    var preview=''+
      '<div class="ds-preview" style="'+esc(styleAttr(vars))+'"><div class="ds-preview-inner">'+
