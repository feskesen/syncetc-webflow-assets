/* syncetc-homepage-v30-aircraft-page-model-v1.js - BEGIN */
(function () {
  "use strict";

  const VERSION = "JS v30-aircraft-page-model-v1";
  const FILE_NAME = "syncetc-homepage-v30-aircraft-page-model-v1.js";
  const MOUNT_ID = "syncetc-webflow-mount";
  const ROOT_ID = "syncetc-v30-root";
  const STYLE_ID = "syncetc-v30-style";
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .syncetc-v30 { min-height:100vh; font-family:Arial, Helvetica, sans-serif; color:var(--aero-text); background:linear-gradient(rgba(240,247,252,.78),rgba(240,247,252,.80)), var(--se-customer-bg); background-position:center; background-size:cover; background-attachment:fixed; padding:10px 0 48px; box-sizing:border-box; }
      .syncetc-v30 * { box-sizing:border-box; }
      .se-v30-version { position:fixed; right:12px; top:12px; z-index:999999; padding:8px 11px; border-radius:999px; background:#12365a; color:#fff; font-size:12px; font-weight:900; box-shadow:0 8px 24px rgba(0,0,0,.25); pointer-events:none; }
      .se-v30-topline { max-width:1180px; margin:0 auto 8px; padding:0 18px; display:flex; justify-content:space-between; align-items:center; gap:10px; color:#12365a; font-size:11px; font-weight:900; }
      .se-v30-topline span { background:rgba(255,255,255,.72); border:1px solid var(--aero-border); border-radius:999px; padding:6px 9px; }
      ...
      /* Update 102 aircraft public page styles - BEGIN */
      and
      /* Update 102 aircraft public page styles - END */
  ...
  function render() {
    ...
    mount.innerHTML = `<div id="${ROOT_ID}" class="syncetc-v30" style="--se-customer-bg:url('${cssUrl(base.backgroundUrl)}')"><div class="se-v30-version">${esc(VERSION)} loaded</div><div class="se-v30-topline"><span>SyncEtc versioned renderer · ${esc(FILE_NAME)}</span><span>${esc(state.customer)} · ${esc(currentPageLabel())}</span></div>${renderWorkbench(base)}${renderHeader(base)}${state.page === 'home' ? renderMarquee(base) : ""}${state.page === 'home' ? renderHome(base) : (state.page === 'info' ? renderInfoPage(base) : (state.page === 'aircraft' ? renderAircraftPage(base) : renderPageModules(base)))}${renderFooter(base)}</div>`;
  }
  ...
  function boot() { render(); bind(); console.log("SYNCETC UPDATE 102 LOADED", VERSION, FILE_NAME); }
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", boot); else boot();
})();
/* syncetc-homepage-v30-aircraft-page-model-v1.js - END */
