/* COMPONENT-master-header-v4.js - BEGIN */
(function () {
  "use strict";
  ...
  var VERSION = "COMPONENT-master-header-v4";
  var wired = false;
  ...
  function softAuthRefresh(reason) {
    try {
      document.dispatchEvent(new CustomEvent("syncetc:auth-soft-change", { detail: { reason: reason || "auth-change" } }));
    } catch (err) {}
  }
  ...
  function signedInNow() {
    ...
    try {
      if (window.SyncEtc && window.SyncEtc.SecurityContext && window.SyncEtc.SecurityContext.getToken) {
        return !!window.SyncEtc.SecurityContext.getToken();
      }
    } catch (err) {}
    return false;
  }

  function watchLoginForSoftRefresh() {
    let ticks = 0;
    const startedSignedIn = signedInNow();
    const timer = setInterval(function () {
      ticks += 1;
      if (!startedSignedIn && signedInNow()) {
        clearInterval(timer);
        softAuthRefresh("login-detected");
        return;
      }
  ...
  function wire() {
    ...
    document.addEventListener("click", async function (e) {
      ...
      if (e.target.closest && e.target.closest("[data-se-auth-logout]")) {
        e.preventDefault();
        try {
          if (window.SyncEtc.SecurityContext && window.SyncEtc.SecurityContext.signOutHard) {
            await window.SyncEtc.SecurityContext.signOutHard();
            return;
          }
...
/* COMPONENT-master-header-v4.js - END */
