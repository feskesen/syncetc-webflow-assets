/* PAGE-INFO-v3.js | customer context config added | Generated: 2026-06-01 */
(function () {
  "use strict";
  ...
  const VERSION = "PAGE-INFO-v3";
  const FILE_NAME = "PAGE-INFO-v3.js";
  ...
  function getSyncEtcConfig() {
    return {
      env: clean(window.SYNCETC_ENV || "production"),
      customerKey: clean(window.SYNCETC_CUSTOMER_KEY || window.SYNCETC_ACTIVE_CUSTOMER_KEY || ""),
      customerName: clean(window.SYNCETC_CUSTOMER_NAME || window.SYNCETC_ACTIVE_CUSTOMER_NAME || ""),
      pageKey: clean(window.SYNCETC_PAGE_KEY || PAGE_KEY),
      hostname: clean(window.location && window.location.hostname)
    };
  }
  ...
  function customerNameFromKey(key) {
    var map = {
      ...
      "demo_flying_club": "150th Aero Flying Club",
  ...
  function renderInfoBody() {
    ...
    var stats = statRows();
    return '<div id="'+ROOT_ID+'" class="syncetc-info-v2" data-page-key="'+PAGE_KEY+'" data-page-access-scope="'+PAGE_ACCESS_SCOPE+'" data-syncetc-customer-key="'+esc(getCustomerKey())+'" data-syncetc-env="'+esc(getSyncEtcConfig().env)+'">'+
      diagnosticHtml()+
