...
(function () {
  ...
  function ensureComponents() {
    ...
    return COMPONENT_FILES.reduce(function (p, file) { return p.then(function () { return loadScriptOnce(base + file + "?v=shared-security-policy-uniformity-1"); }); }, Promise.resolve());
  }
  ...
  function activeCustomerKey() {
    var s = authSnapshot();
    return clean(s.active_customer_key || (s.active_customer && s.active_customer.customer_key));
  }
  ...
  function hasDemoAccess() {
    ...
    return memberships.some(function (m) { return m.customer_key === activeCustomerKey(); });
  }
  ...
  async function uploadPhotoToStorage(photo) {
    ...
    var path = activeCustomerKey() + "/pending/" + photo.galleryItemId + "/" + fileName;
  ...
  async function submitPhotos() {
    ...
    if (!hasDemoAccess()) return setMessage("error", "This login does not have Gallery Submit access for the active customer.");
    ...
    state.modalTitle = "Submitting to " + activeCustomerLabel();
    ...
    try {
      ...
      var response = await fetch(SUBMIT_EDGE_URL, {
        ...
        body: JSON.stringify({
          ...
          payload: { customer_key: activeCustomerKey(), caption: caption, attribution_display_name: credit, photos: uploaded },
          source: VERSION
        })
      });
      var body = await response.json();
      ...
      setMessage("success", "Submission received for " + activeCustomerLabel() + ".");
      render();
    } catch (err) {
      state.busy = false;
  ...
  function renderLoginPanel() {
    return '<span class="se-gs2-label">SyncEtc Login</span><h2>Sign in to continue</h2><p>This page uses the shared Supabase Auth header login.</p><div class="se-gs2-actions"><button type="button" class="se-gs2-btn" onclick="window.SyncEtc && window.SyncEtc.AuthModal && window.SyncEtc.AuthModal.open()">Open Login</button></div>';
  }

  function renderSubmitPanel() {
    var access = canSubmitGallery();
    return '<span class="se-gs2-label">Gallery Submit</span><h2>Submit photos to ' + esc(activeCustomerLabel()) + '</h2><p>Add a caption, use attribution only if someone else created the photo, then submit for review.</p>' +
      ...
      (access ? '' : '<div class="se-gs2-status show error">This login does not have Gallery Submit access for the active customer.</div>') +
  ...
  function init() {
    ensureComponents().then(ensureSupabaseClient).then(function () {
      ...
      if (window.SyncEtc && window.SyncEtc.AuthContext) {
        window.SyncEtc.AuthContext.subscribe(function () { render(); });
        return window.SyncEtc.AuthContext.init().then(refreshSession);
      }
