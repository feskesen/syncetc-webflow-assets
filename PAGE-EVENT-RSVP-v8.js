/* PAGE-EVENT-RSVP-v8.js - BEGIN */
(function(){
"use strict";

var INSTANCE_ID=String(Date.now())+"-"+String(Math.random()).slice(2);
window.__SYNCETC_ACTIVE_RSVP_INSTANCE=INSTANCE_ID;
function isActiveInstance(){return window.__SYNCETC_ACTIVE_RSVP_INSTANCE===INSTANCE_ID;}

var VERSION="PAGE-EVENT-RSVP-v8";
...
function scriptBase(){
  ...
  for(var i=scripts.length-1;i>=0;i--){
    ...
    if(src.indexOf("PAGE-EVENT-RSVP-v8.js")>-1){
...
function loadComponents(){
  ...
  COMPONENT_FILES.forEach(function(file){
    p=p.then(function(){return loadScript(base+file+"?v=event-rsvp-v8");});
  });
  return p;
}
...
function userProfileForPayload(){
  ...
  var email=clean(user.email||profile.email||user.auth_email||user.member_email);
...
function loadRsvps(){
  if(!isActiveInstance())return Promise.resolve();
...
function styleHtml(){
  return '<style id="PAGE-EVENT-RSVP-v8-style">'+
...
function renderMainOnly(){
  ...
  requestAnimationFrame(function(){
    ...
    var html=pageHtml();
    if(html===state.lastRenderHtml)return;
...
function boot(){
  ...
  loadComponents().then(function(){
    ...
    /* Standalone RSVP intentionally does not subscribe to global auth soft-change events during startup.
       Manual Retry RSVP remains available, and login button checks AuthContext directly. */
    ...
    return new Promise(function(resolve){setTimeout(resolve,450);})
      .then(function(){return getAuthToken();})
      .then(function(){
        state.startupLocked=false;
        renderMainOnly();
