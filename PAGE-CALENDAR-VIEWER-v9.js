/* PAGE-CALENDAR-VIEWER-v9.js - BEGIN */
(function(){
"use strict";
...
function ensureComponents(){var base=componentBaseUrl();return COMPONENT_FILES.reduce(function(p,file){return p.then(function(){return loadScriptOnce(base + file + "?v=calendar-viewer-customer-stable-v9");});},Promise.resolve());}
var VERSION="PAGE-CALENDAR-VIEWER-v9",EDGE_READ_URL="https://ocdaohkiwonjmirqkjww.supabase.co/functions/v1/syncetc-calendar-viewer-read";
var state={shell:null,events:[],visibleEvents:[],selectedEvent:null,dateFilter:"upcoming",typeFilter:"all",search:"",loading:true,error:"",lastCustomerKey:"",refreshTimer:null,refreshBusy:false,refreshQueued:false,startupComplete:false,startupFrozen:true,firstRenderDone:false,initialFetchDone:false,initialFallbackTimer:null,lastRefreshAt:0,stableCustomerTimer:null};
...
function scheduleRefresh(reason){
  ...
  if(state.stableCustomerTimer)clearTimeout(state.stableCustomerTimer);
  state.stableCustomerTimer=setTimeout(function(){
    var key=activeCustomerKey();
    if(reason==="customer-hard-change" && key===state.lastCustomerKey){
      return;
    }
    var now=Date.now();
    ...
    state.refreshTimer=setTimeout(function(){runStableRefresh(reason);},250);
  },450);
}
...
function waitForStableCustomerKey(){
  return new Promise(function(resolve){
    var last=activeCustomerKey();
    var stableSince=Date.now();
    var started=Date.now();
    function tick(){
      var nowKey=activeCustomerKey();
      if(nowKey!==last){
        last=nowKey;
        stableSince=Date.now();
      }
      if(nowKey && Date.now()-stableSince>=450){
        state.lastCustomerKey=nowKey;
        resolve(nowKey);
        return;
      }
      if(Date.now()-started>=2500){
        state.lastCustomerKey=nowKey||state.lastCustomerKey||"demo_flying_club";
        resolve(state.lastCustomerKey);
        return;
      }
      setTimeout(tick,100);
    }
...
function initialCalendarLoad(){
  ...
  return waitForStableCustomerKey().then(function(){
    return fetchData();
  }).then(function(){
