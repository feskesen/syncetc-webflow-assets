/* syncetc-homepage-v26-homepage-scale-banner-style-settings.js - BEGIN */
(function () {
  "use strict";

  const VERSION = "JS v26-homepage-scale-banner-style-settings";
  const FILE_NAME = "syncetc-homepage-v26-homepage-scale-banner-style-settings.js";
  const MOUNT_ID = "syncetc-webflow-mount";
  const ROOT_ID = "syncetc-v26-root";
  const STYLE_ID = "syncetc-v26-style";
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .syncetc-v26 { min-height:100vh; font-family:Arial, Helvetica, sans-serif; color:var(--aero-text); background:linear-gradient(rgba(240,247,252,.78),rgba(240,247,252,.80)), var(--se-customer-bg); background-position:center; background-size:cover; background-attachment:fixed; padding:10px 0 48px; box-sizing:border-box; }
      .syncetc-v26 * { box-sizing:border-box; }
      .se-v26-version { position:fixed; right:12px; top:12px; z-index:999999; padding:8px 11px; border-radius:999px; background:#12365a; color:#fff; font-size:12px; font-weight:900; box-shadow:0 8px 24px rgba(0,0,0,.25); pointer-events:none; }
      .se-v26-topline { max-width:1180px; margin:0 auto 8px; padding:0 18px; display:flex; justify-content:space-between; align-items:center; gap:10px; color:#12365a; font-size:11px; font-weight:900; }
      .se-v26-topline span { background:rgba(255,255,255,.72); border:1px solid var(--aero-border); border-radius:999px; padding:6px 9px; }
      ...
      .aero-marquee { max-width:1180px; margin:-4px auto 18px; padding:0 18px; overflow:hidden; }
      .aero-marquee-view { overflow:hidden; position:relative; min-height:58px; display:flex; align-items:center; }
      ...
      .aero-marquee-plane { height:68px; width:auto; margin-right:14px; flex:0 0 auto; filter:drop-shadow(0 5px 7px rgba(12,38,64,.22)); transform:translateY(2px); }
      .aero-marquee-plane.scale-medium { height:52px; }
      .aero-marquee-plane.scale-large { height:68px; }
      .aero-marquee-plane.scale-xl { height:82px; }
      .aero-marquee-text { display:inline-flex; align-items:center; padding:7px 18px; min-height:28px; border-radius:2px; background:rgba(207,220,228,.88); border:1px solid rgba(18,54,90,.34); color:#31465a; font-size:15px; font-weight:900; letter-spacing:.02em; box-shadow:0 4px 10px rgba(12,38,64,.12); }
  ...
  function render() {
    ...
    mount.innerHTML = `<div id="${ROOT_ID}" class="syncetc-v26" style="--se-customer-bg:url('${cssUrl(base.backgroundUrl)}')"><div class="se-v26-version">JS v26 loaded</div><div class="se-v26-topline"><span>SyncEtc versioned renderer · ${esc(FILE_NAME)}</span><span>${esc(state.customer)} · ${esc(currentPageLabel())}</span></div>${renderWorkbench(base)}${renderHeader(base)}${renderMarquee(base)}${state.page === 'home' ? renderHome(base) : renderPageModules(base)}${renderFooter(base)}</div>`;
  }
  ...
  function exportJson() {
    ...
    a.href = url;
    a.download = "syncetc-v26-display-settings-" + Date.now() + ".json";
    document.body.appendChild(a); a.click();
    setTimeout(function(){ URL.revokeObjectURL(url); a.remove(); }, 500);
  }

  function boot() { render(); bind(); console.log("SYNCETC UPDATE 98 LOADED", VERSION, FILE_NAME); }
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", boot); else boot();
})();
/* syncetc-homepage-v26-homepage-scale-banner-style-settings.js - END */
