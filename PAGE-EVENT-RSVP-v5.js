/* PAGE-EVENT-RSVP-v5.js - BEGIN */
(function(){
"use strict";

var VERSION="PAGE-EVENT-RSVP-v5";
...
function scriptBase(){
  ...
  for(var i=scripts.length-1;i>=0;i--){
    ...
    if(src.indexOf("PAGE-EVENT-RSVP-v5.js")>-1){
...
function loadComponents(){
  ...
  COMPONENT_FILES.forEach(function(file){
    p=p.then(function(){return loadScript(base+file+"?v=event-rsvp-v5");});
  });
  return p;
}
  ...
  return getAuthToken().then(function(token){
    ...
    return resp.text().then(function(txt){
      ...
      if(!resp.ok||data.ok===false){
        if(resp.status===401){
          throw new Error("RSVP request failed: 401. Supabase did not accept the current login token. Click Sign in / Refresh Login once, then retry RSVP.");
        }
...
function styleHtml(){
  return '<style id="PAGE-EVENT-RSVP-v5-style">'+
