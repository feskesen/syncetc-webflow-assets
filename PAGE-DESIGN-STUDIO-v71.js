...
/* PAGE-DESIGN-STUDIO-v71.js | replace discard with guarded close | Generated: 2026-06-02 */
(function(){
"use strict";

var VERSION="PAGE-DESIGN-STUDIO-v71";
...
var STYLE_ID="syncetc-design-studio-v71-style";
...
/* v71 guarded close helpers - BEGIN */
function hasAnyPageSpecificUnsavedWork(){
  return !!(state.sideDirty||state.sideAppliedDirty);
}

function confirmClosePageSpecificSettings(){
  if(!hasAnyPageSpecificUnsavedWork())return true;
  return window.confirm("You have unsaved Page Specific Settings changes. Close and lose those changes, or cancel to keep editing?");
}

function forceClosePageSpecificSettings(){
...
function closePageSpecificSettingsWithGuard(){
  if(!confirmClosePageSpecificSettings())return;
  forceClosePageSpecificSettings();
}
/* v71 guarded close helpers - END */
    ...
    '<aside class="ds-side-drawer '+(state.sideOpen?'is-open':'')+'" data-ds-side-drawer>'+
      ...
      '<div class="ds-side-body">'+
        ...
        '<div class="ds-side-actions">'+
          ...
          '<button class="ds-btn secondary" type="button" data-ds-action="close-page-text">Close</button>'+
        '</div>'+
...
/* v71 guarded close capture listener - BEGIN */
if(!window.__SYNCETC_DS_GUARDED_CLOSE_V71){
  window.__SYNCETC_DS_GUARDED_CLOSE_V71=true;
  document.addEventListener("click",function(e){
    var btn=e.target&&e.target.closest?e.target.closest('[data-ds-action="close-page-text"],[data-ds-action="close-page-settings"],[data-ds-action="close-side-drawer"],[data-ds-action="close-drawer"]'):null;
    if(!btn)return;
    e.preventDefault();
    e.stopImmediatePropagation();
    closePageSpecificSettingsWithGuard();
  },true);
}
/* v71 guarded close capture listener - END */


/* v71 guard installer safety shim - BEGIN */
if(typeof installUnsavedGuardsOnce!=="function"){
  function installUnsavedGuardsOnce(){
    if(window.__SYNCETC_DS_GUARD_SHIM_V71)return;
    window.__SYNCETC_DS_GUARD_SHIM_V71=true;
    window.addEventListener("beforeunload",function(e){
      if(!hasAnyPageSpecificUnsavedWork())return;
      e.preventDefault();
      e.returnValue="";
      return "";
    });
...
/* v71 guard installer safety shim - END */


/* v71 guarded close diagnostic - BEGIN */
window.SYNCETC_V71_CLOSE_STATE=function(){
  var out={
    version:VERSION,
    sideOpen:state.sideOpen,
    sideDirty:state.sideDirty,
    sideAppliedDirty:state.sideAppliedDirty,
    hasUnsaved:hasAnyPageSpecificUnsavedWork(),
    closeGuardInstalled:!!window.__SYNCETC_DS_GUARDED_CLOSE_V71
  };
  console.log("SYNCETC_V71_CLOSE_STATE",out);
  return out;
};
/* v71 guarded close diagnostic - END */

installUnsavedGuardsOnce();
/* PAGE-DESIGN-STUDIO-v71.js - END */
