...
(function(){
  ...
  var VERSION = "PAGE-EVENT-ADMIN-v4";
  ...
  function timeOptionsHtml(selected){
    ...
    for(var h=0; h<24; h++){
      for(var m=0; m<60; m+=5){
        ...
        var hour12 = h % 12 || 12;
        var ampm = h < 12 ? "AM" : "PM";
        var label = hour12 + ":" + z2(m) + " " + ampm;
        out.push('<option value="'+v+'" '+(v===selected?'selected':'')+'>'+label+'</option>');
      }
    }
    return out.join("");
  }
  ...
  function fetchData(){
    return Promise.all([
      fetchRest("syncetc_calendar_events?customer_key=eq."+encodeURIComponent(activeCustomerKey())+"&select=*&order=start_at.asc.nullslast"),
      fetchRest("syncetc_calendar_event_types?customer_key=eq."+encodeURIComponent(activeCustomerKey())+"&select=*&order=sort_order.asc"),
      fetchRest("syncetc_calendar_locations?customer_key=eq."+encodeURIComponent(activeCustomerKey())+"&select=*&order=sort_order.asc"),
      fetchRest("syncetc_calendar_rsvps?customer_key=eq."+encodeURIComponent(activeCustomerKey())+"&select=*")
  ...
  function renderCreateTab(){
    ...
    return '<section class="ea-panel"><div class="ea-toolbar"><div><h2>'+(isEdit?'Edit Event':'Create Event')+'</h2><p>Use a blank event, a saved event type/location, or preselect from a prior event.</p></div><div class="ea-toolbar-right"><button type="button" class="ea-btn" data-ea-clear-form>Blank Event</button><button type="button" class="ea-btn primary" data-ea-save-event>'+(isEdit?'Save Changes':'Create Event')+'</button></div></div>'+ 
      ...
      '<div class="ea-grid two"><label class="ea-field"><span>New Event Type Name</span><input class="ea-input" id="ea-new-type-name" placeholder="Only if this is a new type"></label><div class="ea-field"><span>Save Type</span><button type="button" class="ea-btn ea-conditional-save" id="ea-save-new-type-inline" data-ea-save-new-type-from-event hidden>Save as New Event Type</button></div></div>'+ 
      '<div class="ea-grid four"><label class="ea-field"><span>Start Date</span><input class="ea-input" id="ea-start-date" type="date" value="'+esc(toDateInput(ev.start_at))+'"></label><label class="ea-field" id="ea-start-time-field"><span>Start Time</span><select class="ea-select" id="ea-start-time">'+timeOptionsHtml(toTime5Value(ev.start_at))+'</select></label><label class="ea-field" id="ea-end-date-field"><span>End Date</span><input class="ea-input" id="ea-end-date" type="date" value="'+esc(toDateInput(ev.end_at || ev.start_at))+'"></label><label class="ea-field" id="ea-end-time-field"><span>End Time</span><select class="ea-select" id="ea-end-time">'+timeOptionsHtml(toTime5Value(ev.end_at || ev.start_at))+'</select></label></div>'+ 
      ...
      '<label class="ea-field"><span>Address</span><input class="ea-input" id="ea-address" value="'+esc(ev.address)+'"></label><div class="ea-inline-tools"><button type="button" class="ea-btn" data-ea-event-map-from-address>Get Google Maps URL from location/address</button><button type="button" class="ea-btn ea-conditional-save" id="ea-save-new-location-inline" data-ea-save-new-location-from-event hidden>Save as New Location</button></div>'+ 
      ...
      '<div class="ea-actions"><button type="button" class="ea-btn primary" data-ea-save-event>'+(isEdit?'Save Changes':'Create Event')+'</button><button type="button" class="ea-btn" data-ea-clear-form>Reset Form</button></div>'+ 
    '</section>';
  }
  ...
  function readLocationForm(){
    return { customer_key:activeCustomerKey(), location_id: state.editingLocationId || uid("loc"), name: val("ea-location-name"), slug: slugify(val("ea-location-name")), location_label: val("ea-location-label") || val("ea-location-name"), address: val("ea-location-address"), map_url: val("ea-location-map"), venue_note: val("ea-location-note"), active:true, sort_order:999 };
  }
  ...
  function renderBody(){
    ...
    body += renderTabs()+'<main class="ea-main"><div class="ea-message">'+esc(state.message)+'</div>';
  ...
  function bind(){
    ...
    document.addEventListener("click", function(e){
      ...
      if(el.closest && el.closest("[data-ea-save-new-type-from-event]")){ var tf=readTypeFromEventForm(); if(!tf.name){ state.message="Enter a new event type name first."; rerender(); return; } sendAction("create_event_type", tf); return; }
      if(el.closest && el.closest("[data-ea-save-new-location-from-event]")){ var lf=readLocationFromEventForm(); if(!lf.name){ state.message="Enter a location name first."; rerender(); return; } sendAction("create_location", lf); return; }
      ...
      if(el.closest && el.closest("[data-ea-save-type]")){ var tf2=readTypeForm(); if(!tf2.name){ state.message="Event Type Name is required."; rerender(); return; } sendAction(state.editingTypeId?"update_event_type":"create_event_type", tf2); return; }
      ...
      if(el.closest && el.closest("[data-ea-save-location]")){ var lf2=readLocationForm(); if(!lf2.name){ state.message="Location Name is required."; rerender(); return; } sendAction(state.editingLocationId?"update_location":"create_location", lf2); return; }
