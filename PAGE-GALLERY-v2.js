/* PAGE-GALLERY-v2.js - BEGIN */
(function () {
  "use strict";

  var VERSION = "PAGE-GALLERY-v2";
  ...
  function renderToolbar() {
    return '<div class="se-gallery-toolbar">' +
      ...
      '<div class="se-gallery-tools">' +
        ...
        '</select>' +
        (state.sort === "random" ? '<button type="button" class="se-gallery-btn" data-se-gallery-shuffle>Shuffle</button>' : '') +
      '</div>' +
    '</div>';
  }
  ...
  function bind() {
    document.addEventListener("click", function (e) {
      ...
      var shuffle = e.target.closest && e.target.closest("[data-se-gallery-shuffle]");
      if (shuffle) {
        state.items.forEach(function (item) { item.randomRank = Math.random(); });
        state.page = 1;
        renderPage();
        return;
      }
    ...
    document.addEventListener("change", function (e) {
      if (e.target && e.target.id === "se-gallery-sort") {
        state.sort = e.target.value || "random";
        state.page = 1;
        renderPage();
      }
    });
